...
- (void)serialize:(NSMutableData *)data
{
    ...
    for (std::vector<TGImageSizeRecord>::iterator it = sizes.begin(); it != sizes.end(); it++)
    {
        ...
        //qi.zhang modify, 兼容32位和64位设备，图片宽度和高度转化为32位整形进行存储
        int32_t width = floor(it->size.width);
        int32_t height = floor(it->size.height);
...
+ (TGImageInfo *)deserialize:(NSInputStream *)is
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        int32_t width = 0;
        int32_t height = 0;
        [is read:(uint8_t *)&width maxLength:4];
        [is read:(uint8_t *)&height maxLength:4];
        size.width = width;
        size.height = height;
