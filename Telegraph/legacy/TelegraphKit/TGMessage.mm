...
/*#ifdef DEBUG
    ...
    NSString *textEscaped = [TGReusableLabel escapeAtMessage:_text];
    
    if (textEscaped.length < 3 || textEscaped.length > 1024 * 20)
        return nil;
    ...
    if (_textCheckingResults == nil && !_hasNoCheckingResults)
    {
        ...
        const unichar *stringCharacters = CFStringGetCharactersPtr((__bridge CFStringRef)textEscaped);
        int length = textEscaped.length;
        ...
        else
        {
            ...
            unichar (*characterAtIndexImp)(id, SEL, NSUInteger) = (typeof(characterAtIndexImp))[textEscaped methodForSelector:sel];
            
            for (int i = 0; i < length; i++)
            {
                unichar c = characterAtIndexImp(textEscaped, sel, i);
        ...
        if (containsSomething)
        {
            ...
            [dataDetector enumerateMatchesInString:textEscaped options:0 range:NSMakeRange(0, textEscaped.length) usingBlock:^(NSTextCheckingResult *match, __unused NSMatchingFlags flags, __unused BOOL *stop)
            {
...
{
    NSString *pointStr = @"";
    TGVoteInfoObject *voteInfo = [self.contentProperties objectForKey:StoreKey_MessageVoteInfo];
    if (voteInfo.count != 0) {
        NSString *key = (voteInfo.count>1 || voteInfo.count<-1)?@"Vote.Points":@"Vote.Point";
        pointStr = [NSString stringWithFormat:@"%d %@  ",voteInfo.count,TGLocalized(key)];
    }
    return pointStr;
}
