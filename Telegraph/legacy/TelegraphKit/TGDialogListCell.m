...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier assetsSource:(id<TGDialogListCellAssetsSource>)assetsSource
{
    ...
    if (self)
    {
        ...
        _unreadView = [[UIView alloc] initWithFrame:CGRectMake(_avatarView.frame.size.width - 8, 0, 8, 8)];
        ...
        [_avatarView addSubview:_unreadView];
...
{
    //qi.zhang add,stick change background color
    if (self.isSticked) {
        self.backgroundColor = UIColorRGB(0xf2f2f6);
        _textView.backgroundColor = UIColorRGB(0xf2f2f6);
    }else{
...
- (void)setShowUnreadView:(BOOL)showUnreadView
{
    if (_showUnreadView != showUnreadView) {
        _showUnreadView = showUnreadView;
        _unreadView.hidden = !_showUnreadView;
    }
}
