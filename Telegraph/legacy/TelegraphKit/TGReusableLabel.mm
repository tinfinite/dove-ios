...
+ (TGReusableLabelLayoutData *)calculateLayout:(NSString *)text additionalAttributes:(NSArray *)additionalAttributes textCheckingResults:(NSArray *)textCheckingResults font:(CTFontRef)font textColor:(UIColor *)textColor frame:(CGRect)frame orMaxWidth:(float)maxWidth flags:(int)flags textAlignment:(UITextAlignment)textAlignment outIsRTL:(bool *)outIsRTL additionalTrailingWidth:(CGFloat)additionalTrailingWidth
{
    ...
    float fontLineSpacing = floorf(fontLineHeight * 1.2f);   //qi.zhang modify from 1.12
    ...
    BOOL ret = [self isContainAt:text];
    
    text = [self escapeAtMessage:text];
    ...
    CGColorRef defaultLinkColor = nil;
    
    if (CGColorEqualToColor(textColor.CGColor, [UIColor whiteColor].CGColor)) {
        defaultLinkColor = textColor.CGColor;
    }else{
    ...
    if (flags & TGReusableLabelLayoutMultiline)
    {
        ...
        rect.size.height += 4;   //qi.zhang modify text padding
        ...
        if (flags & TGReusableLabelLayoutDateSpacing)
        {
            ...
            if (textLines.count == 1)
            {
                ...
                //qi.zhang modify short line padding
                rect.size.height += 3;
                //qi.zhang modify over
            }
...
+ (NSString *)escapeAtMessage:(NSString *)text
{
    ...
    NSString *escapeString = text;
    
    for (NSTextCheckingResult *match in matches) {
        NSRange matchRange = [match rangeAtIndex:1];
        NSString *matchString = [text substringWithRange:matchRange];
        NSInteger startLocation = [matchString rangeOfString:@"["].location;
        NSInteger endLocation = [matchString rangeOfString:@"]"].location;
        NSString *subString = [matchString substringWithRange:NSMakeRange(startLocation+1, endLocation - startLocation-1)];
        
        TGUser *user = [TGDatabaseInstance() loadUser:[@([subString intValue]) int32Value]];
        if (user) {
            subString = [NSString stringWithFormat:@"%@%@",user.firstName,user.lastName];
            if ([subString isChinese]) {
                subString = [NSString stringWithFormat:@"@%@%@",user.firstName,user.lastName];
            }else if (user.firstName.length == 0){
                subString = [NSString stringWithFormat:@"@%@",user.lastName];
            }else if (user.lastName.length == 0){
                subString = [NSString stringWithFormat:@"@%@",user.firstName];
            }else{
                subString = [NSString stringWithFormat:@"@%@ %@",user.firstName,user.lastName];
            }
            escapeString = [escapeString stringByReplacingOccurrencesOfString:matchString withString:subString];
        }else{
            subString = [matchString substringToIndex:startLocation];
            if ([[[subString substringFromIndex:1] stringByReplacingOccurrencesOfString:@" " withString:@""] isChinese]) {
...
+ (BOOL)isContainAt:(NSString *)text
{
    NSString *pattern = @"(@.*?\\[\\d+\\])";
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        NSInteger startLocation = [matchString rangeOfString:@"["].location;
        NSInteger endLocation = [matchString rangeOfString:@"]"].location;
        NSString *subString = [matchString substringWithRange:NSMakeRange(startLocation+1, endLocation - startLocation)];
        if ([subString intValue] == TGTelegraphInstance.clientUserId) {
            return YES;
        }
    }
    return NO;
