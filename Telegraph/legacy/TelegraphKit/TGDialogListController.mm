...
- (void)_loadStatusViews
{
    if (_titleStatusLabel == nil)
    {
        ...
        _titleStatusIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [T8GroupHttpRequestService getAllGroupBoardUnreadCount:[TGDatabaseInstance() loadAllGroupConversationIds] success:^(NSDictionary *dictRet) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        NSArray *data = dictRet[@"data"];
        [data enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
            NSDictionary *dict = (NSDictionary *)obj;
            NSString *conversationId = [NSString stringWithFormat:@"-%@",obj[@"tg_group_id"]];
            [TGDatabaseInstance() storeConversationInfoWithId:[conversationId integerValue] unreadCount:[dict[@"count"] integerValue]];
        }];
...
- (void)prepareCell:(TGDialogListCell *)cell forConversation:(TGConversation *)conversation animated:(bool)animated isSearch:(bool)isSearch
{
    ...
    if (cell.reuseTag != (NSInteger)conversation || cell.conversationId != conversation.conversationId)
    {
        ...
        cell.isSticked = [[T8StickToTopManager sharedInstance] checkConversationIsStickedWithID:[NSString stringWithFormat:@"%lld",conversation.conversationId]];
        
        [cell resetView:animated];
    }
    
    cell.showUnreadView = [TGDatabaseInstance() getUnreadCountWithConversationId:conversation.conversationId];
