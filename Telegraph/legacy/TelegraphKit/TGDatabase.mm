...
static NSString *_liveUnreadCountDispatchPathForGroup = nil;
static NSString *_liveUnreadCountDispatchPathForSingle = nil;
...
@property (nonatomic) int t8SchemaVersion;
@property (nonatomic, strong) NSString *nodesTableName;
@property (nonatomic, strong) NSString *streamsTableName;
@property (nonatomic, strong) NSString *t8blockUsersTableName;
...
+ (void)setLiveUnreadCountDispatchPathForGroup:(NSString *)path
{
    _liveUnreadCountDispatchPathForGroup = path;
}

+ (void)setLiveUnreadCountDispatchPathForSingle:(NSString *)path
{
    _liveUnreadCountDispatchPathForSingle = path;
}
...
- (id)init
{
    ...
    if (self != nil)
    {
        ...
        _nodesTableName = [NSString stringWithFormat:@"nodes_v%d", _t8SchemaVersion];
        _streamsTableName = [NSString stringWithFormat:@"streams_v%d", _t8SchemaVersion];
        _t8blockUsersTableName = [NSString stringWithFormat:@"t8blocked_users_v%d", _t8SchemaVersion];
    }
    return self;
}
...
- (void)upgradeTables
{
    ...
    //qi.zhang add
    for (int i = _t8SchemaVersion - 2; i < _t8SchemaVersion + 2; i++) {
        if (i != _t8SchemaVersion) {
            [_database executeUpdate:[NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", [NSString stringWithFormat:@"nodes_v%d", i]]];
            [_database executeUpdate:[NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", [NSString stringWithFormat:@"streams_v%d", i]]];
            [_database executeUpdate:[NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", [NSString stringWithFormat:@"t8blocked_users_v%d",i]]];
        }
    }
    ...
    [_database executeUpdate:[NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (node_id STRING, source_type INTEGER, is_public INTEGER, total_score INTEGER, total_reply INTEGER, create_at STRING, upvote INTEGER, downvote INTEGER, author BLOB, post BLOB, forward BLOB, stream_type INTEGER, PRIMARY KEY(node_id, stream_type))", _nodesTableName]];
    [_database executeUpdate:[NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (stream_id INTEGER PRIMARY KEY, node_ids BLOB)", _streamsTableName]];
    [_database executeUpdate:[NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (t8_id STRING, tg_id INTEGER)", _t8blockUsersTableName]];
}
...
- (void)dropDatabase:(bool)fullDrop
{
    [self dispatchOnDatabaseThread:^
    {
        ...
        else
        {
            ...
            [_database executeUpdate:[NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", _nodesTableName]];
            [_database executeUpdate:[NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", _streamsTableName]];
            [_database executeUpdate:[NSString stringWithFormat:@"DROP TABLE IF EXISTS %@", _t8blockUsersTableName]];
        }
...
static inline void storeNodeToDatabase(TGDatabase *database, TGNodeModel *node, StreamType streamType)
{
    NSString *queryFormat = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (node_id, source_type, is_public, total_score, total_reply, create_at, upvote, downvote, author, post, forward, stream_type) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", database.nodesTableName];
    
    NSData *authorData = [NSJSONSerialization dataWithJSONObject:node.author.dataDict options:kNilOptions error:nil];
    NSData *postData = nil;
    if (node.post.dataDict) {
        postData = [NSJSONSerialization dataWithJSONObject:node.post.dataDict options:kNilOptions error:nil];
    }
    NSData *forwardData = nil;
    if (node.forward.dataDict) {
        forwardData = [NSJSONSerialization dataWithJSONObject:node.forward.dataDict options:kNilOptions error:nil];
    }
    [database.database executeUpdate:queryFormat, node.nodeId, @(node.sourceType), @(node.isPublic), @(node.totalScore), @(node.totalReply), node.createTime, @(node.isUpvote), @(node.isDownvote), authorData, postData, forwardData, @(streamType)];
}
...
static inline TGNodeModel *loadNodeFromDatabase(FMResultSet *result)
{
    TGNodeModel *node = [[TGNodeModel alloc] init];
    
    node.synchronize = NO;
    node.nodeId = [result stringForColumn:@"node_id"];
    node.sourceType = [result intForColumn:@"source_type"];
    node.isPublic = [result intForColumn:@"is_public"];
    node.totalScore = [result intForColumn:@"total_score"];
    node.totalReply = [result intForColumn:@"total_reply"];
    node.createTime = [result stringForColumn:@"create_at"];
    node.isUpvote = [result boolForColumn:@"upvote"];
    node.isDownvote = [result boolForColumn:@"downvote"];
    node.synchronize = YES;
    
    NSData *authorData = [result dataForColumn:@"author"];
    if (authorData) {
        NSDictionary *authorDic = [NSJSONSerialization JSONObjectWithData:authorData options:kNilOptions error:nil];
        node.author = [[TGNodeAuthorModel alloc] initWithDict:authorDic];
    }
    
    NSData *postData = [result dataForColumn:@"post"];
    if (postData) {
        NSDictionary *postDic = [NSJSONSerialization JSONObjectWithData:postData options:kNilOptions error:nil];
        node.post = [[TGNodePostModel alloc] initWithDict:postDic];
    }
    
    NSData *forwardData = [result dataForColumn:@"forward"];
    if (forwardData) {
        NSDictionary *forwardDic = [NSJSONSerialization JSONObjectWithData:forwardData options:kNilOptions error:nil];
        node.forward = [[TGNodeForwardModel alloc] initWithDict:forwardDic];
    }
...
- (void)storeConversationPrivilegeWithId:(int64_t)conversationId privilege:(GroupDiscoverPrivilege)privilege
{
    NSString *pri = @(privilege).stringValue;
    [self setConversationCustomProperty:conversationId name:murMurHash32(@"conversationPrivilege") value:[pri dataUsingEncoding:NSUTF8StringEncoding]];
}

- (GroupDiscoverPrivilege)getPrivilegeWithConversationId:(int64_t)conversationId
{
    GroupDiscoverPrivilege privilege = GroupDiscoverPrivilegePrivate;
    NSData *privilegeData = [self conversationCustomPropertySync:conversationId name:murMurHash32(@"conversationPrivilege")];
    if (privilegeData) {
        NSInteger p = [[NSString alloc] initWithData:privilegeData encoding:NSUTF8StringEncoding].integerValue;
        if (p == 0) {
...
- (void)storeConversationInfoWithId:(int64_t)conversationId privilege:(GroupDiscoverPrivilege)privilege description:(NSString *)description language:(NSString *)language
{
    if (privilege != GroupDiscoverPrivilegeUnknow) {
        NSString *pri = @(privilege).stringValue;
        [self setConversationCustomProperty:conversationId name:murMurHash32(@"conversationPrivilege") value:[pri dataUsingEncoding:NSUTF8StringEncoding]];
    }
    
    if (description!=nil && description.length>0) {
        [self setConversationCustomProperty:conversationId name:murMurHash32(@"conversationDescription") value:[description dataUsingEncoding:NSUTF8StringEncoding]];
    }

    if (language!=nil && language.length>0) {
        [self setConversationCustomProperty:conversationId name:murMurHash32(@"conversationLanguage") value:[language dataUsingEncoding:NSUTF8StringEncoding]];
    }
}

- (BOOL)getAnonymousWithConversationId:(int64_t)conversationId
{
    BOOL anonymous = NO;
    NSData *anonymousData = [self conversationCustomPropertySync:conversationId name:murMurHash32(@"conversationAnonymous")];
    if (anonymousData) {
        anonymous = [[NSString alloc] initWithData:anonymousData encoding:NSUTF8StringEncoding].boolValue;
    }
    return anonymous;
}

- (void)storeConversationInfoWithId:(int64_t)conversationId anonymous:(BOOL)anonymous
{
    NSString *anony = @(anonymous).stringValue;
    [self setConversationCustomProperty:conversationId name:murMurHash32(@"conversationAnonymous") value:[anony dataUsingEncoding:NSUTF8StringEncoding]];
}

- (void)storeConversationInfoWithId:(int64_t)conversationId groupBoardStatus:(GroupBoardStatus)status
{
    NSString *st = @(status).stringValue;
    [self setConversationCustomProperty:conversationId name:murMurHash32(@"conversationGroupBoardStatus") value:[st dataUsingEncoding:NSUTF8StringEncoding]];
}

- (GroupBoardStatus)getGroupBoardStatusWithConversationId:(int64_t)conversationId
{
    GroupBoardStatus status = GroupBoardStatusPublic;
    NSData *statusData = [self conversationCustomPropertySync:conversationId name:murMurHash32(@"conversationGroupBoardStatus")];
    if (statusData) {
        status = [[[NSString alloc] initWithData:statusData encoding:NSUTF8StringEncoding] integerValue];
    }
    return status;
}

- (NSString *)getDescriptionWithConversationId:(int64_t)conversationId
{
    NSString *description = @"";
    NSData *descriptionData = [self conversationCustomPropertySync:conversationId name:murMurHash32(@"conversationDescription")];
    if (descriptionData) {
        description = [[NSString alloc] initWithData:descriptionData encoding:NSUTF8StringEncoding];
    }
    return description;
}

- (NSString *)getLanguageWithConversationId:(int64_t)conversationId
{
    NSString *language = @"";
    NSData *languageData = [self conversationCustomPropertySync:conversationId name:murMurHash32(@"conversationLanguage")];
    if (languageData) {
        language = [[NSString alloc] initWithData:languageData encoding:NSUTF8StringEncoding];
    }
    return language;
}

- (NSDictionary *)loadChatUnreadCountInfo
{
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    __block int countForGroup = 0;
    __block int countForSingle = 0;
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"SELECT cid,unread_count FROM %@", _conversationListTableName];
        FMResultSet *result = [_database executeQuery:queryFormat];
        while ([result next]) {
            int64_t cid = [result intForColumn:@"cid"];
            int unreadCount = [result intForColumn:@"unread_count"];
            if ([self isGroup:cid]) {
                countForGroup += unreadCount;
            }else{
                countForSingle += unreadCount;
            }
        }
        
    } synchronous:true];
    [dict setObject:@(countForGroup) forKey:@"countForGroup"];
    [dict setObject:@(countForSingle) forKey:@"countForSingle"];
    return [dict copy];
}

- (void)storeNodeList:(NSArray *)nodes streamType:(StreamType)streamType
{
    [self dispatchOnDatabaseThread:^{
        [_database beginTransaction];
        
        for (TGNodeModel *node in nodes) {
            storeNodeToDatabase(self, node, streamType);
        }
...
{
    NSArray *nodes = [self loadNodesWithIDs:@[nodeId] streamType:streamType];
    if (nodes.count > 0) {
...
- (NSArray *)loadNodesWithIDs:(NSArray *)nodeIdArray streamType:(StreamType)streamType
{
    ...
    [self dispatchOnDatabaseThread:^{
        ...
        //查询被屏蔽的用户
        NSMutableArray *blockedT8IDs = [NSMutableArray array];
        NSMutableArray *blockedTGIDs = [NSMutableArray array];
        FMResultSet *blockResult = [_database executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@", _t8blockUsersTableName]];
        while ([blockResult next]) {
            [blockedT8IDs addObject:[blockResult stringForColumn:@"t8_id"]];
            [blockedTGIDs addObject:@([blockResult intForColumn:@"tg_id"]).stringValue];
        }
        
        //查询node
        __block NSString *nodeIdStr = @"";
        [nodeIdArray enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL __unused *stop) {
            NSString *nodeid = (NSString *)obj;
            if (idx == 0) {
                nodeIdStr = [nodeIdStr stringByAppendingFormat:@"'%@'",nodeid];
            }else{
                nodeIdStr = [nodeIdStr stringByAppendingFormat:@",'%@'",nodeid];
            }
        }];
        FMResultSet *nodeResult = [_database executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE node_id in (%@) AND stream_type=?", _nodesTableName, nodeIdStr], @(streamType)];
        while ([nodeResult next]) {
            TGNodeModel *node = loadNodeFromDatabase(nodeResult);
            if ([blockedT8IDs containsObject:node.author.authorId] || [blockedTGIDs containsObject:node.author.tgUserId]) {
                continue;
            }
...
- (void)deleteNodeWithID:(NSString *)nodeId
{
    [self dispatchOnDatabaseThread:^{
        ...
        NSString *queryFormat = [NSString stringWithFormat:@"DELETE FROM %@ WHERE node_id=?", _nodesTableName];
        [_database executeUpdate:queryFormat,nodeId];
        
    } synchronous:false];
}

- (void)storeNodeInfoWithID:(NSString *)nodeId reply:(NSInteger)reply
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"UPDATE %@ SET total_reply=? WHERE node_id=?", _nodesTableName];
        [_database executeUpdate:queryFormat, @(reply), nodeId];
        
    } synchronous:false];
}

- (void)storeNodeInfoWithID:(NSString *)nodeId score:(NSInteger)score
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"UPDATE %@ SET total_score=? WHERE node_id=?", _nodesTableName];
        [_database executeUpdate:queryFormat, @(score), nodeId];
        
    } synchronous:false];
}

- (void)storeNodeInfoWithID:(NSString *)nodeId upvote:(BOOL)upvote
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"UPDATE %@ SET upvote=? WHERE node_id=?", _nodesTableName];
        [_database executeUpdate:queryFormat, @(upvote), nodeId];
        
    } synchronous:false];
}

- (void)storeNodeInfoWithID:(NSString *)nodeId downvote:(BOOL)downvote
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"UPDATE %@ SET downvote=? WHERE node_id=?", _nodesTableName];
        [_database executeUpdate:queryFormat, @(downvote), nodeId];
        
    } synchronous:false];
}

- (void)storeStreamWithConversationId:(int64_t)conversationId andStreamType:(StreamType)streamType nodeIDs:(NSArray *)nodeIDs
{
    [self dispatchOnDatabaseThread:^{
        NSString *queryFormat = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (stream_id, node_ids) VALUES (?, ?)", _streamsTableName];
        
        int64_t streamId = -1;
        if (streamType == StreamTypeGroup) {
            streamId = ABS(conversationId);
        }
        
        NSData *nodeIDData = nil;
        if (nodeIDs) {
            nodeIDData = [NSJSONSerialization dataWithJSONObject:nodeIDs options:kNilOptions error:nil];
        }
        
        [_database executeUpdate:queryFormat, @(streamId), nodeIDData];
    } synchronous:false];
}

- (void)insertIDToStream:(int64_t)conversationId andStreamType:(StreamType)streamType nodeID:(NSString *)nodeID
{
    [self dispatchOnDatabaseThread:^{
        ...
        NSArray *nodeIdArr = nil;
        NSString *query = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE stream_id=?", _streamsTableName];
        FMResultSet *result = [_database executeQuery:query, @(streamId)];
        if ([result next]) {
            NSData *nodeIDData = [result dataForColumn:@"node_ids"];
            if (nodeIDData) {
                nodeIdArr = [NSJSONSerialization JSONObjectWithData:nodeIDData options:kNilOptions error:nil];
            }
        }
        
        NSMutableArray *tmpArr = [NSMutableArray array];
        if (nodeIdArr) {
            [tmpArr addObjectsFromArray:nodeIdArr];
        }
        [tmpArr insertObject:nodeID atIndex:0];
        
        NSString *update = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (stream_id, node_ids) VALUES (?, ?)", _streamsTableName];
        
        NSData *nodeIDData = nil;
        nodeIDData = [NSJSONSerialization dataWithJSONObject:tmpArr options:kNilOptions error:nil];
        
        [_database executeUpdate:update, @(streamId), nodeIDData];
        
    } synchronous:false];
}

- (NSArray *)loadNodeIDsWithConversationId:(int64_t)conversationId andStreamType:(StreamType)streamType
{
    __block NSArray *nodeIDs = [NSArray array];
    ...
    [self dispatchOnDatabaseThread:^{
        ...
        FMResultSet *nodeResult = [_database executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@ WHERE stream_id=?", _streamsTableName], @(streamId)];
        if ([nodeResult next]) {
            NSData *nodeIDData = [nodeResult dataForColumn:@"node_ids"];
            if (nodeIDData) {
                nodeIDs = [NSJSONSerialization JSONObjectWithData:nodeIDData options:kNilOptions error:nil];
            }
        }
...
- (void)coverBlockedUsers:(NSArray *)users
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"DELETE FROM %@", _t8blockUsersTableName];
        [_database executeUpdate:queryFormat];
        
        [_database beginTransaction];

        for (NSDictionary *userDict in users) {
            NSString *t8id = [userDict stringForKey:@"t8_user_id" withDefault:@""];
            NSString *tgid = [userDict stringForKey:@"tg_user_id" withDefault:@""];
            
            if (t8id.length>0 || tgid.length>0) {
                NSString *queryFormat = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (t8_id, tg_id) VALUES (?, ?)", _t8blockUsersTableName];
                [_database executeUpdate:queryFormat, t8id, @(tgid.integerValue)];
            }
        }
...
- (void)blockUserWithT8id:(NSString *)t8id tgid:(int)tgid
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (t8_id, tg_id) VALUES (?, ?)", _t8blockUsersTableName];
        [_database executeUpdate:queryFormat, t8id, @(tgid)];
        
    } synchronous:false];
}

- (void)unblockUserWithT8id:(NSString *) __unused t8id tgid:(int)tgid
{
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"DELETE FROM %@ WHERE tg_id=?", _t8blockUsersTableName];
        [_database executeUpdate:queryFormat, @(tgid)];
        
    } synchronous:false];
}

- (void)filterBlockedNodes:(NSMutableArray *)nodes
{
    [self dispatchOnDatabaseThread:^{
        ...
        NSMutableArray *blockedNodes = [NSMutableArray array];
        [nodes enumerateObjectsUsingBlock:^(id obj, NSUInteger __unused idx, BOOL __unused *stop) {
            TGNodeModel *node = (TGNodeModel *)obj;
            if ([blockedT8IDs containsObject:node.author.authorId] || [blockedTGIDs containsObject:node.author.tgUserId]) {
                [blockedNodes addObject:node];
            }
        }];
        [nodes removeObjectsInArray:blockedNodes];
        
    } synchronous:true];
}

- (BOOL)checkBlockWithT8id:(NSString *)t8id tgid:(int)tgid
{
    __block BOOL blocked = false;
    
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE t8_id=? OR tg_id=?", _t8blockUsersTableName];
        FMResultSet *blockResult = [_database executeQuery:queryFormat, t8id, @(tgid)];
        while ([blockResult next]) {
            if (tgid == [blockResult intForColumn:@"tg_id"]) {
                blocked = true;
                break;
            }
        }
...
- (void)checkBlock
{
    [self dispatchOnDatabaseThread:^{
        ...
        NSString *queryFormat = [NSString stringWithFormat:@"SELECT * FROM %@", _t8blockUsersTableName];
        FMResultSet *blockResult = [_database executeQuery:queryFormat];
        while ([blockResult next]) {
            NSString *t8id = [blockResult stringForColumn:@"t8_id"];
            NSInteger tgid = [blockResult intForColumn:@"tg_id"];
            NSLog(@"t8:%@,   tg:%ld", t8id, (long)tgid);
        }
        
    } synchronous:false];
}

- (NSArray *)loadGroupConversationInfo
{
    NSMutableArray *groupInfos = [NSMutableArray array];
    
    [self dispatchOnDatabaseThread:^{
        
        NSString *queryFormat = [NSString stringWithFormat:@"SELECT * FROM %@ WHERE cid<0", _conversationListTableName];
        FMResultSet *conversationResult = [_database executeQuery:queryFormat];
        TGConversation *conversation = [[TGConversation alloc] init];
        while ([conversationResult next]) {
            NSString *chatTitle = [conversationResult stringForColumn:@"chat_title"];
            int64_t cid = [conversationResult intForColumn:@"cid"];
            if (chatTitle.length>0) {
                NSMutableDictionary *groupDict = [NSMutableDictionary dictionary];
                [groupDict setObject:@(ABS(cid)).stringValue forKey:@"id"];
                [groupDict setObject:chatTitle forKey:@"name"];
                [conversation deserializeChatPhoto:[conversationResult dataForColumn:@"chat_photo"]];
                [groupDict setObject:conversation.chatPhotoSmall forKey:@"image_key"];
                [groupInfos addObject:groupDict];
            }
        }
        
    } synchronous:true];
    
    return [groupInfos copy];
}

- (NSArray *)loadAllGroupConversationIds
{
    NSMutableArray *groupInfos = [NSMutableArray array];
    ...
    [self dispatchOnDatabaseThread:^{
        ...
        while ([conversationResult next]) {
            ...
            if (chatTitle.length>0)
            {
                [groupInfos addObject:@(ABS(cid)).stringValue];
            }
        }
...
- (NSInteger)getUnreadCountWithConversationId:(int64_t)conversationId
{
    NSInteger count = 0;
    NSData *anonymousData = [self conversationCustomPropertySync:conversationId name:murMurHash32(@"groupBoardUnreadCount")];
    if (anonymousData) {
        count = [[NSString alloc] initWithData:anonymousData encoding:NSUTF8StringEncoding].integerValue;
    }
    return count;
}

- (void)storeConversationInfoWithId:(int64_t)conversationId unreadCount:(NSInteger)count
{
    NSString *anony = @(count).stringValue;
    [self setConversationCustomProperty:conversationId name:murMurHash32(@"groupBoardUnreadCount") value:[anony dataUsingEncoding:NSUTF8StringEncoding]];
}

//- (NSArray *)loadMyPostsWithLimit:(NSInteger)limit
//{
//    NSMutableArray *nodeArray = [NSMutableArray array];
//    
//    [self dispatchOnDatabaseThread:^{
...
- (void)applyPts:(int)pts date:(int)date seq:(int)seq qts:(int)qts unreadCount:(int)unreadCount unreadCountForGroup:(int)unreadCountForGroup unreadCountForSingle:(int)unreadCountForSingle
{
    [self dispatchOnDatabaseThread:^
    {
        ...
        if (unreadCountForGroup > 0)
            currentState.unreadCountForGroup = unreadCountForGroup;
        if (unreadCountForSingle > 0)
            currentState.unreadCountForSingle = unreadCountForSingle;
        
        [self setPts:currentState.pts date:currentState.date seq:currentState.seq qts:currentState.qts unreadCount:currentState.unreadCount unreadCountForGroup:currentState.unreadCountForGroup unreadCountForSingle:currentState.unreadCountForSingle];
    } synchronous:false];
}

- (void)setPts:(int)pts date:(int)date seq:(int)seq qts:(int)qts unreadCount:(int)unreadCount unreadCountForGroup:(int)unreadCountForGroup unreadCountForSingle:(int)unreadCountForSingle
{   
    [self dispatchOnDatabaseThread:^
    {
        int lastUnreadCount = 0;
        int lastUnreadCountForGroup = 0;
        int lastUnreadCountForSingle = 0;
        ...
        lastUnreadCountForGroup = _cachedDatabaseState.unreadCountForGroup;
        _cachedDatabaseState.unreadCountForGroup = unreadCountForGroup;
        lastUnreadCountForSingle = _cachedDatabaseState.unreadCountForSingle;
        _cachedDatabaseState.unreadCountForSingle = unreadCountForSingle;
        ...
        if (lastUnreadCountForGroup != unreadCountForGroup && _liveUnreadCountDispatchPathForGroup != nil)
        {
            [ActionStageInstance() dispatchOnStageQueue:^
             {
                 [ActionStageInstance() dispatchResource:_liveUnreadCountDispatchPathForGroup resource:[[SGraphObjectNode alloc] initWithObject:[[NSNumber alloc] initWithInt:unreadCountForGroup]]];
             }];
        }
        if (lastUnreadCountForSingle != unreadCountForSingle && _liveUnreadCountDispatchPathForSingle != nil)
        {
            [ActionStageInstance() dispatchOnStageQueue:^
             {
                 [ActionStageInstance() dispatchResource:_liveUnreadCountDispatchPathForSingle resource:[[SGraphObjectNode alloc] initWithObject:[[NSNumber alloc] initWithInt:unreadCountForSingle]]];
             }];
        }
        ...
        [data appendBytes:&unreadCountForGroup length:4];
        [data appendBytes:&unreadCountForSingle length:4];
...
- (void)setUnreadCount:(int)unreadCount
{
    [self dispatchOnDatabaseThread:^
    {
        ...
        [self setPts:state.pts date:state.date seq:state.seq qts:state.qts unreadCount:state.unreadCount unreadCountForGroup:state.unreadCountForGroup unreadCountForSingle:state.unreadCountForSingle];
    } synchronous:false];
}
...
- (void)setUnreadCountForGroup:(int)unreadCount
{
    [self dispatchOnDatabaseThread:^{
        TGDatabaseState state = [self databaseState];
        state.unreadCountForGroup = unreadCount;
        [self setPts:state.pts date:state.date seq:state.seq qts:state.qts unreadCount:state.unreadCount unreadCountForGroup:state.unreadCountForGroup unreadCountForSingle:state.unreadCountForSingle];
    } synchronous:false];
}
...
- (void)setUnreadCountForSingle:(int)unreadCount
{
    [self dispatchOnDatabaseThread:^{
        TGDatabaseState state = [self databaseState];
        state.unreadCountForSingle = unreadCount;
        [self setPts:state.pts date:state.date seq:state.seq qts:state.qts unreadCount:state.unreadCount unreadCountForGroup:state.unreadCountForGroup unreadCountForSingle:state.unreadCountForSingle];
    } synchronous:false];
}
...
{   
    ...
    [self dispatchOnDatabaseThread:^
    {
        ...
        if (value == nil || value.length < 4 * 4)
        {
            ...
            state.unreadCount = 0;
            state.qts = 0;
            state.unreadCountForGroup = 0;
            state.unreadCountForSingle = 0;
        }
        else
        {
            unsigned int ptr = 0;
            ...
            int unreadCountForGroup = 0;
            if (value.length >= ptr + 4) {
                [value getBytes:&unreadCountForGroup range:NSMakeRange(ptr, 4)];
                ptr += 4;
            }
            
            int unreadCountForSingle = 0;
            if (value.length >= ptr + 4) {
                [value getBytes:&unreadCountForSingle range:NSMakeRange(ptr, 4)];
                ptr += 4;
            }
            ...
            state.unreadCount = unreadCount;
            state.qts = qts;
            state.unreadCountForGroup = unreadCountForGroup;
            state.unreadCountForSingle = unreadCountForSingle;
        }
        ...
        if (dispatch)
        {
            ...
            if (unreadCount != 0)
            {
                ...
                if ([self isGroup:conversationId]) {
                    int newUnreadCountForGroup = [self databaseState].unreadCountForGroup + unreadCount;
                    [self setUnreadCountForGroup:MAX(newUnreadCountForGroup, 0)];
                }else{
                    int newUnreadCountForSingle = [self databaseState].unreadCountForSingle + unreadCount;
                    [self setUnreadCountForSingle:MAX(newUnreadCountForSingle, 0)];
                }
            }
        }
...
- (void)deleteMessages:(NSArray *)mids populateActionQueue:(bool)populateActionQueue fillMessagesByConversationId:(NSMutableDictionary *)messagesByConversationId keepDate:(bool)keepDate populateActionQueueIfIncoming:(bool)populateActionQueueIfIncoming
{
    [self dispatchOnDatabaseThread:^
    {
        ...
        int deletedUnreadCountForGroup = 0;
        int deletedUnreadCountForSingle = 0;
        ...
        for (NSNumber *nMid in mids)
        {
            ...
            if ([messageResult next])
            {
                ...
                if ([nMid intValue] < TGMessageLocalMidBaseline && [messageResult intForColumn:@"outgoing"] == 0 && [messageResult intForColumn:@"unread"] != 0)
                {
                    ...
                    deletedUnreadCount++;
                    if ([self isGroup:cid]) {
                        deletedUnreadCountForGroup++;
                    }else{
                        deletedUnreadCountForSingle++;
                    }
                }
                else
                {
        ...
        if (deletedUnreadCountForGroup != 0) {
            int unreadCount = [self databaseState].unreadCountForGroup - deletedUnreadCountForGroup;
            [self setUnreadCountForGroup:MAX(unreadCount, 0)];
        }
        
        if (deletedUnreadCountForSingle != 0) {
            int unreadCount = [self databaseState].unreadCountForSingle - deletedUnreadCountForSingle;
            [self setUnreadCountForSingle:MAX(unreadCount, 0)];
        }
    ...
    {   
        ...
        if (conversation != nil)
        {
            ...
            if (conversation.unreadCount != 0)
            {
                ...
                [self setUnreadCount:MAX(unreadCount, 0)];
                if ([self isGroup:conversationId]) {
                    int unreadCountForGroup = [self databaseState].unreadCountForGroup - conversation.unreadCount;
                    [self setUnreadCountForGroup:MAX(unreadCountForGroup, 0)];
                }else{
                    int unreadCountForSingle = [self databaseState].unreadCountForSingle - conversation.unreadCount;
                    [self setUnreadCountForSingle:MAX(unreadCountForSingle, 0)];
                }
            }
...
- (void)markMessagesAsRead:(NSArray *)mids
{
    ...
    [self dispatchOnDatabaseThread:^
    {
        ...
        int completeReadCountForGroup = 0;
        int completeReadCountForSingle = 0;
        for (std::map<int64_t, int>::iterator it = unreadByConversation.begin(); it != unreadByConversation.end(); it++)
        {
            completeReadCount += it->second;
            if ([self isGroup:it->first]) {
                completeReadCountForGroup += it->second;
            }else{
                completeReadCountForSingle += it->second;
            }
        ...
        if (completeReadCountForGroup > 0) {
            [self setUnreadCountForGroup:MAX(0, [self databaseState].unreadCountForGroup - completeReadCountForGroup)];
        }
        if (completeReadCountForSingle > 0) {
            [self setUnreadCountForSingle:MAX(0, [self databaseState].unreadCountForSingle - completeReadCountForSingle)];
        }
    } synchronous:false];
}
...
- (BOOL)storeVoteInfoForMessage:(int32_t)messageId count:(int)count upVote:(BOOL)upVoted downVote:(BOOL)downVoted
{
    TGMessage *message = [self loadMessageWithMid:messageId];
    NSMutableDictionary *contentProperty = [message.contentProperties mutableCopy];
    if (contentProperty == nil) {
        contentProperty = [NSMutableDictionary dictionary];
    }
    TGVoteInfoObject *obj = [contentProperty objectForKey:StoreKey_MessageVoteInfo];
    if (obj == nil) {
        obj = [[TGVoteInfoObject alloc] init];
    }
    
    BOOL result = true;
    if (obj.count == count && obj.upvote == upVoted && obj.downvote == downVoted) {
        result = false;
        return result;
    }
    ...
    if (count != INT_MIN) {
        ...
        BOOL oldUpVoted = obj.upvote;
        BOOL oldDownVoted = obj.downvote;
        int count = obj.count;
        if (upVoted) {    //变成赞
            if (oldDownVoted) {
                count += 2;
    ...
    obj.upvote = upVoted;
    obj.downvote = downVoted;
    [contentProperty setObject:obj forKey:StoreKey_MessageVoteInfo];
    [self replaceContentPropertiesInMessageWithId:messageId contentProperties:[contentProperty copy]];
    return result;
}

- (BOOL)storeVoteInfoForMessageWithKey:(NSString *)messageKey count:(int)count upVote:(BOOL)upVoted downVote:(BOOL)downVoted
{
    NSArray *params = [messageKey componentsSeparatedByString:@"_"];
    if (params.count != 4) {
        return NO;
    }
    int64_t fromUid = [[params objectAtIndex:0] intValue];
    int64_t toUid = 0 - [[params objectAtIndex:1] intValue];
    NSTimeInterval date = [[params objectAtIndex:2] doubleValue];
    NSUInteger index = [[params objectAtIndex:3] intValue];
    NSArray *messages = [self getSameSecondMessageWithFromUid:fromUid toUid:toUid date:date];
    ...
    return [self storeVoteInfoForMessage:message.mid count:count upVote:upVoted downVote:downVoted];
}

- (NSArray *)getSameSecondMessageWithFromUid:(int64_t)fromUid toUid:(int64_t)toUid date:(NSTimeInterval)date
{
    NSMutableArray *array = [[NSMutableArray alloc] init];
    
    [self dispatchOnDatabaseThread:^
     {
         FMResultSet *result = [_database executeQuery:[[NSString alloc] initWithFormat:@"SELECT * FROM %@ WHERE cid=? AND from_id=? AND date=?", _messagesTableName], @(toUid), @(fromUid), @(date)];
         while ([result next])
         {
             [array addObject:loadMessageFromQueryResult(result)];
         }
     } synchronous:true];
...
- (TGMessage *)getSameSecondMessageWithFromUid:(int64_t)fromUid toUid:(int64_t)toUid date:(NSTimeInterval)date index:(NSUInteger)index
{
    NSArray *array = [self getSameSecondMessageWithFromUid:fromUid toUid:toUid date:date];
...
- (void)readHistoryOptimized:(int64_t)conversationId populateActionQueue:(bool)populateActionQueue completion:(void (^)(bool))completion
{
    [self dispatchOnDatabaseThread:^
    {
        ...
        if (conversationData != nil && ((!conversationData.outgoing && conversationData.unread) || conversationData.unreadCount != 0 || conversationData.serviceUnreadCount != 0))
        {
            ...
            [self setUnreadCount:MAX(unreadCount, 0)];
            if ([self isGroup:conversationId]) {
                int unreadCountForGroup = [self databaseState].unreadCountForGroup - conversationData.unreadCount;
                [self setUnreadCountForGroup:MAX(unreadCountForGroup, 0)];
            }else{
                int unreadCountForSingle = [self databaseState].unreadCountForSingle - conversationData.unreadCount;
                [self setUnreadCountForSingle:MAX(unreadCountForSingle, 0)];
            }
...
- (BOOL)isGroup:(int64_t)conversationID
{
    if (conversationID<0 && conversationID>INT_MIN) {
        return YES;
    }else{
        return NO;
    }
