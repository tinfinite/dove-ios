...
#pragma mark - getter
- (NSDictionary *)chatModelDict
{
    _chatModelDict = nil;
    return @{@"title": TGLocalized(@"Conversation.SecretChat"), @"action": @"secretChat"};
}
...
- (void)_createOrUpdatePrimaryTitlePanel:(bool)createIfNeeded
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone)
    {
        ...
        else
        {
            ...
            [actions addObject:self.chatModelDict];
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    ...
    else if ([action isEqualToString:@"titlePanelAction"])
    {
        ...
        else if ([panelAction isEqualToString:@"secretChat"]){
//            [self _controllerAvatarPressed];
            [self startSecretChatPressed];
        }
    }
...
- (void)actorCompleted:(int)status path:(NSString *)path result:(id)result
{
    ...
    else if ([path hasPrefix:@"/tg/encrypted/createChat/"])
    {
        TGDispatchOnMainThread(^
        {
            ...
            {
                TGUser *user = [TGDatabaseInstance() loadUser:_uid];
                [[[TGAlertView alloc] initWithTitle:nil message:status == -2 ? [[NSString alloc] initWithFormat:TGLocalized(@"Profile.CreateEncryptedChatOutdatedError"), user.displayFirstName, user.displayFirstName] : TGLocalized(@"Profile.CreateEncryptedChatError") delegate:nil cancelButtonTitle:TGLocalized(@"Common.OK") otherButtonTitles:nil] show];
            }
        });
    }
