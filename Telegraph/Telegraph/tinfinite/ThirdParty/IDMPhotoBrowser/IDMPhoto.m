...
//            NSURLRequest *request = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:@"http://tinfinite.qiniudn.com/dove/image/a/f20dd6c040433eeefd56778a63ada39dw500h302"]
//                                                          cachePolicy:NSURLRequestReturnCacheDataElseLoad
//                                                      timeoutInterval:0];

            
            UIImage *cache = [[UIImageView sharedImageCache] cachedImageForRequest:request];
            if (cache) {
                self.underlyingImage = cache;
                [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
            }else{
                AFHTTPRequestOperation *op = [[AFHTTPRequestOperation alloc] initWithRequest:request];
                op.responseSerializer = [OLImageResponseSerializer serializer];
                
                [op setCompletionBlockWithSuccess:^(AFHTTPRequestOperation __unused *operation, id __unused responseObject) {
                    UIImage *image = responseObject;
                    self.underlyingImage = image;
                    [self performSelectorOnMainThread:@selector(imageLoadingComplete) withObject:nil waitUntilDone:NO];
                } failure:^(AFHTTPRequestOperation __unused *operation, NSError __unused *error) { }];
                
                [op setDownloadProgressBlock:^(NSUInteger __unused bytesRead, long long totalBytesRead, long long totalBytesExpectedToRead) {
