...
//        _statusLabel.textColor = UIColorRGB(0x787878);
        _statusLabel.textColor = UIColorRGBA(0xffffff, 0.7f);
        _statusLabel.font = TGSystemFontOfSize(12.0f);
...
//            _statusLabel.textColor = _statusHasAccentColor ? UIColorRGB(0x007bff) : UIColorRGB(0x86868d);
            _statusLabel.textColor = _statusHasAccentColor ? UIColorRGB(0xffffff) : UIColorRGB(0xffffff);
        }
    }
}
...
- (void)setIcons:(NSArray *)icons
{
    ...
    for (TGModernConversationTitleIcon *icon in icons)
    {
        ...
        {
            iconView = [[UIImageView alloc] initWithImage:[icon.image imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate]];
...
- (void)setModalProgressStatus:(NSString *)modalProgressStatus
{
    if (!TGStringCompare(_modalProgressStatus, modalProgressStatus))
    {
        ...
        if (_titleModalProgressLabel == nil)
        {
            ...
            _titleModalProgressIndicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
        }
...
#warning TODO take back title as an argument
        ...
        CGFloat buttonsWidth = _editingMode ? (clearAllButtonSize + cancelButtonSize) : (backButtonSize + avatarButtonSize + 60);
