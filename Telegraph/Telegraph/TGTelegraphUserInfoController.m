...
@interface TGTelegraphUserInfoController () <TGAlertSoundControllerDelegate, TGUserInfoEditingPhoneCollectionItemDelegate, TGPhoneLabelPickerControllerDelegate, TGCreateContactControllerDelegate, TGAddToExistingContactControllerDelegate>
{
    ...
    TGUserInfoVariantCollectionItem *_stickItem;
    
    TGCollectionMenuSection *_blockUserSection;
    TGUserInfoButtonCollectionItem *_blockPostItem;
...
@property (nonatomic,assign) BOOL postBlocked;
...
- (instancetype)initWithUid:(int32_t)uid withoutActions:(bool)withoutActions sharedMediaPeerId:(int64_t)sharedMediaPeerId sharedMediaOptions:(NSDictionary *)sharedMediaOptions
{
    ...
    if (self != nil)
    {
        ...
        _stickItem = [[TGUserInfoVariantCollectionItem alloc] initWithTitle:TGLocalized(@"GroupInfo.Stick") variant:nil action:@selector(stickPressed)];
        _stickItem.deselectAutomatically = true;
        _sharedMediaSection = [[TGCollectionMenuSection alloc] initWithItems:@[_sharedMediaItem,_stickItem]];
        
        _blockPostItem = [[TGUserInfoButtonCollectionItem alloc] initWithTitle:TGLocalized(@"Stream.BlockPost") action:@selector(blockPostPressed)];
        _blockPostItem.titleColor = [UIColor redColor];
        _blockPostItem.deselectAutomatically = true;
        self.postBlocked = [TGDatabaseInstance() checkBlockWithT8id:@"" tgid:_uid];
        [self updatePostBlock:self.postBlocked];
        _blockUserItem = [[TGUserInfoButtonCollectionItem alloc] initWithTitle:TGLocalized(@"Conversation.BlockUser") action:@selector(blockUserPressed)];
        _blockUserItem.titleColor = [UIColor redColor];
        _blockUserItem.deselectAutomatically = true;
        _blockUserSection = [[TGCollectionMenuSection alloc] initWithItems:@[_blockPostItem, _blockUserItem]];
...
#pragma mark - getter
- (NSString *)stickConversationID
{
    if (!_stickConversationID) {
        _stickConversationID = [NSString stringWithFormat:@"%d",_uid];
    }
    return _stickConversationID;
}
...
- (void)_updatePhonesAndActions
{
    ...
    if (_editing)
    {
        ...
        if (blockUserSectionIndex != NSNotFound) {
            [self.menuSections deleteSection:blockUserSectionIndex];
        }
...
//    [_stickItem setVariant:[[T8StickToTopManager sharedT8StickToTopManager] checkIsStickedWithConversationID:0 andUserID:_user.uid]?TGLocalized(@"UserInfo.NotificationsEnabled") : TGLocalized(@"UserInfo.NotificationsDisabled")];
    [_stickItem setVariant:[[T8StickToTopManager sharedInstance] checkConversationIsStickedWithID:self.stickConversationID]?TGLocalized(@"UserInfo.NotificationsEnabled") : TGLocalized(@"UserInfo.NotificationsDisabled")];
...
- (void)blockPostPressed
{
    BOOL blocked = !self.postBlocked;
    
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setObject:@"" forKey:@"t8id"];
    [dict setObject:@(_uid).stringValue forKey:@"tgid"];
    if (blocked) {
        [TGDatabaseInstance() blockUserWithT8id:@"" tgid:_uid];
        [[NSNotificationCenter defaultCenter] postNotificationName:Notification_Key_User_Blocked object:dict];
    }else{
        [TGDatabaseInstance() unblockUserWithT8id:@"" tgid:_uid];
        [[NSNotificationCenter defaultCenter] postNotificationName:Notification_Key_User_Unblocked object:dict];
    }
    
    [T8NodeHttpRequestService blockUserWithTGID:@(_uid).stringValue andT8ID:@"" blockAction:blocked success:^(NSDictionary __unused *dictRet) {
        
    } failure:^(NSDictionary __unused *dictRet, NSError __unused *error) {
        
    }];
    
    [self updatePostBlock:blocked];
}

- (void)blockUserPressed
{
    bool blocked = !self.isBlocked;
    [ActionStageInstance() dispatchOnStageQueue:^
     {
...
- (void)reportUserPressed
{
    ...
    TGActionSheet *actionSheet = [[TGActionSheet alloc] initWithTitle:nil actions:actions actionBlock:^(TGTelegraphUserInfoController *controller, NSString *action)
                                  {
...
- (void)reportRequestWithReason:(NSString *)reportReason
{
    [T8ReportHttpRequestService reportWithTargetId:@(_uid).stringValue reportType:ReportTypeUser reason:reportReason successBlock:^(NSDictionary __unused *dictRet) {
...
- (void)stickPressed
{
    [[[TGActionSheet alloc] initWithTitle:nil actions:@[
      ...
      {
          if ([action isEqualToString:@"enable"]){
//              [[T8StickToTopManager sharedT8StickToTopManager] stickUserWithUserID:_user.uid action:YES];
              [[T8StickToTopManager sharedInstance] stickConversationWithID:self.stickConversationID action:YES];
          }else if ([action isEqualToString:@"disable"]){
//              [[T8StickToTopManager sharedT8StickToTopManager] stickUserWithUserID:_user.uid action:NO];
              [[T8StickToTopManager sharedInstance] stickConversationWithID:self.stickConversationID action:NO];
          }
          [controller _updateNotificationSettings:NO];
      } target:self] showInView:self.view];
}
...
- (void)actionStageResourceDispatched:(NSString *)path resource:(id)resource arguments:(id)arguments
{
    ...
    else if ([path isEqualToString:@"/tg/blockedUsers"])
    {
        [self actorCompleted:ASStatusSuccess path:path result:resource];
    }
...
- (void)updateUserBlocked:(bool)blocked
{
    ...
    NSString *title = @"";
    if (blocked) {
        title = TGLocalized(@"Conversation.UnblockUser");
    }else{
        title = TGLocalized(@"Conversation.BlockUser");
    }
    TGUserInfoButtonCollectionItemView *view = (TGUserInfoButtonCollectionItemView *)_blockUserItem.boundView;
    if (view) {
        [view setTitle:title];
    }else{
        _blockUserItem.title = title;
    }
}

- (void)updatePostBlock:(BOOL)blocked
{
    self.postBlocked = blocked;
    NSString *title = @"";
    if (blocked) {
        title = TGLocalized(@"Stream.UnblockPost");
    }else{
        title = TGLocalized(@"Stream.BlockPost");
    }
    TGUserInfoButtonCollectionItemView *view = (TGUserInfoButtonCollectionItemView *)_blockPostItem.boundView;
    if (view) {
        [view setTitle:title];
    }else{
        _blockPostItem.title = title;
    }
}
