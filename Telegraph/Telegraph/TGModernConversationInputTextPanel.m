...
- (instancetype)initWithFrame:(CGRect)frame accessoryView:(UIView *)panelAccessoryView
{
    ...
    if (self)
    {
        ...
        _keyboardButton = [[TGModernButton alloc] initWithFrame:CGRectMake(9, 11, attachImage.size.width, attachImage.size.height)];
        _keyboardButton.exclusiveTouch = true;
        [_keyboardButton setImage:[UIImage imageNamed:@"face_btn"] forState:UIControlStateNormal];
        ...
        _attachButton = [[TGModernButton alloc] initWithFrame:CGRectMake(9 + attachImage.size.width + 9, 11, attachImage.size.width, attachImage.size.height)];
...
{
    BOOL emoji = self.inputField.internalTextView.inputView != nil;
    if (emoji) {
        [self.keyboardButton setImage:[UIImage imageNamed:@"face_btn"] forState:UIControlStateNormal];
    }else{
        [self.keyboardButton setImage:[UIImage imageNamed:@"text_btn"] forState:UIControlStateNormal];
    }
    
    id<TGModernConversationInputTextPanelDelegate> delegate = (id<TGModernConversationInputTextPanelDelegate>)self.delegate;
    if ([delegate respondsToSelector:@selector(inputPanelRequestedKeyboardSwitch:emojiBoard:)])
        [delegate inputPanelRequestedKeyboardSwitch:self emojiBoard:!emoji];
}
...
//            insets = UIEdgeInsetsMake(9.0f, 41.0f, 8.0f, 0.0f);
            insets = UIEdgeInsetsMake(9.0f, 81.0f, 8.0f, 0.0f);   //qi.zhang modify
        else
...
- (void)layoutSubviews
{
    ...
    setViewFrame(_keyboardButton, CGRectMake(TGIsPad() ? (1.0f) : (-TGRetinaPixel + 3), frame.size.height - [self baseHeight], inputFieldClippingFrame.origin.x/2 - 1.0f, [self baseHeight] - (TGIsPad() ? 1 : 0)));
    setViewFrame(_attachButton, CGRectMake(TGIsPad() ? (1.0f) : (-TGRetinaPixel + [self baseHeight] - 5.0f), frame.size.height - [self baseHeight], inputFieldClippingFrame.origin.x/2 - 1.0f, [self baseHeight] - (TGIsPad() ? 1 : 0)));
...
- (void)growingTextViewDidPressReturnKeyTypeSend:(HPGrowingTextView *)__unused grouwingTextView
{
