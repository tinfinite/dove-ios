...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        _anonymousItem = [[TGSwitchCollectionItem alloc] initWithTitle:TGLocalized(@"Settings.AnonymousItem") isOn:!T8CONTEXT.anonymous];
        ...
        TGCollectionMenuSection *lastSeenSection = [[TGCollectionMenuSection alloc] initWithItems:@[
            ...
            _blockedUsersItem,
            _anonymousItem,
            [[TGCommentCollectionItem alloc] initWithText:TGLocalized(@"Settings.AnonymousInfo")],
            _lastSeenItem,
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    ...
    {
        if (options[@"item"] == _anonymousItem)
        {
            BOOL anonymous = ![options[@"value"] boolValue];
            [T8GroupAndCommunityService updateUserAnonymousInfo:anonymous success:^(NSDictionary __unused *dictRet) {
                T8CONTEXT.anonymous = anonymous;
            } fail:^(NSDictionary __unused *dictRet, NSError __unused *error) {
                [T8HudHelper showHUDMessage:TGLocalized(@"Settings.SettingFailed")];
            }];
        }
    }
}
