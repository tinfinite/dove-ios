...
- (void)execute:(NSDictionary *)options
{
    ...
    for (NSNumber *nConversationId in messagesByConversation)
    {
        ...
        //qi.zhang modify, fix 64bit problem, when in secret chat, minRemoteMid would be convert to unsigned long(NSUInteger), and it crash
...
//            [TGDatabaseInstance() fillConversationHistoryHole:[nConversationId longLongValue] indexSet:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(minRemoteMid, maxRemoteMid - minRemoteMid)]];
            [TGDatabaseInstance() fillConversationHistoryHole:[nConversationId longLongValue] indexSet:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange((unsigned int)minRemoteMid, (unsigned int)(maxRemoteMid - minRemoteMid))]];
