...
- (void)_controllerGroupBoardPressed
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone)
    {
        TGGroupInfoBoardController *groupInfoBoardController = [[TGGroupInfoBoardController alloc] initWithConversationId:_conversationId];
        
        TGModernConversationController *controller = self.controller;
        [controller.navigationController pushViewController:groupInfoBoardController animated:true];
    }
    else
    {
        TGModernConversationController *controller = self.controller;
        if (controller != nil)
        {
            TGGroupInfoBoardController *groupInfoController = [[TGGroupInfoBoardController alloc] initWithConversationId:_conversationId];
...
- (void)_controllerSelectAtController:(NSString *)text
{
    if (text.length > 0) {
    ...
    if ([text isEqualToString:@"@"]) {
        ...
        TGSelectAtUserController *selectAtUser = [[TGSelectAtUserController alloc] initWithConversationId:self.conversationId];
        selectAtUser.delegate = controller;
        [controller.navigationController pushViewController:selectAtUser animated:YES];
    }
}
...
- (void)_createOrUpdatePrimaryTitlePanel:(bool)createIfNeeded
{
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone)
    {
        ...
        [actions addObject:@{@"title": TGLocalized(@"GroupInfo.GroupBoard"), @"action": @"board"}];
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    if ([action isEqualToString:@"titlePanelAction"])
    {
        ...
        else if ([panelAction isEqualToString:@"board"])
        {
            [self _controllerGroupBoardPressed];
        }
