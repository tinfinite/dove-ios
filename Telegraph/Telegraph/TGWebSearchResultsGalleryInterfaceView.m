...
- (void)setDoneButtonTextType:(RecentItemTextType)textType
{
    if (textType == RecentItemTextTypeSend) {
        [_doneButton setTitle:TGLocalized(@"MediaPicker.Send") forState:UIControlStateNormal];
    }else if (textType == RecentItemTextTypeAttach){
        [_doneButton setTitle:TGLocalized(@"MediaPicker.Attach") forState:UIControlStateNormal];
    }
    [_doneButton sizeToFit];
    _doneButton.frame = CGRectMake(_toolbarView.frame.size.width - _doneButton.frame.size.width, 0, _doneButton.frame.size.width, 44);
}
