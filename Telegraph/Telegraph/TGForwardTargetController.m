...
- (id)initWithForwardMessages:(NSArray *)forwardMessages sendMessages:(NSArray *)sendMessages
{
    ...
    if (self)
    {
        ...
        _dialogListCompanion.companionType = TGTelegraphDialogListCompanionTypeAll;
...
- (id)initWithQRCodeImage:(UIImage *)qrCodeImage
{
    self = [super init];
    if (self)
    {
...
- (void)loadView
{
    ...
    else
    {
        ...
        backgroundView.backgroundColor = UIColorRGB(0xf7f7f7);
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex != alertView.cancelButtonIndex && _selectedTarget != nil)
    {
        ...
        else
        {
            ...
            else if (_qrCodeImage != nil){
                if ([_selectedTarget isKindOfClass:[TGUser class]])
                {
                    TGUser *user = (TGUser *)_selectedTarget;
                    [[TGInterfaceManager instance] navigateToConversationWithId:user.uid conversation:nil performActions:@{@"forwardQRCode": _qrCodeImage} animated:false];
                }
                else if ([_selectedTarget isKindOfClass:[TGConversation class]])
                {
                    TGConversation *conversation = (TGConversation *)_selectedTarget;
                    [[TGInterfaceManager instance] navigateToConversationWithId:conversation.conversationId conversation:nil performActions:@{@"forwardQRCode": _qrCodeImage} animated:false];
                }
            }
            else
            {
