...
- (instancetype)initWithMessage:(TGMessage *)message imageInfo:(TGImageInfo *)imageInfo author:(TGUser *)author context:(TGModernViewContext *)context
{
    ...
    if (self != nil)
    {
        ...
        NSString *pointStr = [NSString stringWithFormat:@"%@%@",[message getPointsString],[TGDateUtils stringForShortTime:(int)message.date]];
        [_imageModel setTimestampString:pointStr displayCheckmarks:!_incoming && _deliveryState != TGMessageDeliveryStateFailed checkmarkValue:(_incoming ? 0 : ((_deliveryState == TGMessageDeliveryStateDelivered ? 1 : 0) + (_read ? 1 : 0))) animated:false];
