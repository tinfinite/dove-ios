...
+ (void)beginEarlyInitialization
{
    ...
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        [TGDatabase setLiveUnreadCountDispatchPathForGroup:@"/tg/unreadCountForGroup"];
        [TGDatabase setLiveUnreadCountDispatchPathForSingle:@"/tg/unreadCountForSingle"];
...
{
    [self configGoogleAnalyticsParams];
    ...
    [(TGApplication *)application forceSetStatusBarStyle:UIStatusBarStyleLightContent animated:false];
    ...
    TGTelegraphDialogListCompanion *dialogListCompanionGroup = [[TGTelegraphDialogListCompanion alloc] init];
    dialogListCompanionGroup.showBroadcastsMenu = true;
    dialogListCompanionGroup.companionType = TGTelegraphDialogListCompanionTypeGroup;
    _groupDialogListController = [[TGGroupDialogListViewController alloc] initWithCompanion:dialogListCompanionGroup];
    
    _nodeStreamController = [[TGNodeStreamController alloc] init];
    
    TGTelegraphDialogListCompanion *dialogListCompanionSingle = [[TGTelegraphDialogListCompanion alloc] init];
    dialogListCompanionSingle.showBroadcastsMenu = true;
    dialogListCompanionSingle.companionType = TGTelegraphDialogListCompanionTypeSingle;
    _singleDialogListController = [[TGSingleChatListController alloc] initWithCompanion:dialogListCompanionSingle];
    ...
    _meController = [[TGMeController alloc] initWithUid:0];
    
    printStartupCheckpoint(5);
    
    _mainTabsController = [[TGMainTabsController alloc] init];
    [_mainTabsController setViewControllers:[NSArray arrayWithObjects:_groupDialogListController, _nodeStreamController, _singleDialogListController,  _meController, nil]];
    ...
    //dispatch_async(dispatch_get_main_queue(), ^
    {
        ...
        [(id<ASWatcher>)_groupDialogListController.dialogListCompanion actorCompleted:ASStatusSuccess path:@"/tg/dialoglist/(0)" result:node];
        [(id<ASWatcher>)_singleDialogListController.dialogListCompanion actorCompleted:ASStatusSuccess path:@"/tg/dialoglist/(0)" result:node];
    ...
    else
    {
        ...
        if (clearControllerStates)
        {
            ...
            dispatch_after(popTime, dispatch_get_main_queue(), ^
            {
                ...
                [_groupDialogListController.dialogListCompanion clearData];
                [_singleDialogListController.dialogListCompanion clearData];
...
- (void)presentMainController
{
    ...
    if (user.userName && user.userName.length==0) {
        TGUsernameController *usernameController = [[TGUsernameController alloc] initWithType:UsernameControllerTypeLogin];
...
- (void)loadSettings
{
    ...
    if ((value = [userDefaults objectForKey:@"autoGesturePassword"]) != nil)
        _autoGesturePassword = [value boolValue];
    else
        _autoGesturePassword = false;
...
- (void)saveSettings
{
    ...
    [userDefaults setObject:[NSNumber numberWithBool:_autoGesturePassword] forKey:@"autoGesturePassword"];
...
#warning TODO reset back id
            ...
            if (_mainTabsController.selectedIndex != 0)
                [_mainTabsController setSelectedIndex:0];
        }
    }
}
...
- (void)application:(UIApplication *)__unused application didReceiveRemoteNotification:(NSDictionary *)userInfo
{
    ...
    [self processT8RemoteNotification:userInfo];
...
- (void)processT8RemoteNotification:(NSDictionary *)userInfo
{
    if ([[userInfo allKeys] containsObject:@"t"])
    {
        NotificationType type = [userInfo[@"t"] integerValue];
        switch (type) {
            case NotificationTypeComment:
            {
                [[NSNotificationCenter defaultCenter] postNotificationName:Notification_Key_ReceiveComment object:nil];
            }
                break;
            case NotificationTypeUpvote:
            {
                [[NSNotificationCenter defaultCenter] postNotificationName:Notification_Key_ReceiveUpvote object:nil];
            }
                break;
            case NotificationTypeReplyGroup:
            {
...
{
    TGMeController *accountSettingsController = [[TGMeController alloc] initWithUid:uid];
    
    int index = -1;
    for (id controller in _mainTabsController.viewControllers)
    {
        index++;
        if ([controller isKindOfClass:[TGMeController class]])
            break;
    }
    ...
    if (index != -1)
    {
        ...
        _meController = accountSettingsController;
    }
}
...
{
    if (TGAppDelegateInstance.autoGesturePassword) {
        GesturePasswordController *gesturePasswordVC= [[GesturePasswordController alloc] initWithGesturePasswordType:GesturePasswordTypeDefault uid:TGTelegraphInstance.clientUserId];
        [self.window.rootViewController presentViewController:gesturePasswordVC animated:NO completion:nil];
    }
}

- (void)configGoogleAnalyticsParams
{
    [GAI sharedInstance].trackUncaughtExceptions = YES;
    [[GAI sharedInstance].logger setLogLevel:kGAILogLevelVerbose];
    [GAI sharedInstance].dispatchInterval = 20;
    id tracker = [[GAI sharedInstance] trackerWithTrackingId:@"UA-60803712-3"];
    
    NSString *version = [[NSBundle mainBundle] objectForInfoDictionaryKey:(NSString *)kCFBundleVersionKey];
    [tracker set:kGAIAppVersion value:version];
    [tracker set:kGAISampleRate value:@"100.0"];
}

- (void)bindTifiniteUser
{
    if (TGTelegraphInstance.clientUserId != 0)
    {
        TGUser *user = [TGDatabaseInstance() loadUser:TGTelegraphInstance.clientUserId];
