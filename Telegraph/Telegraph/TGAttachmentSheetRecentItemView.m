...
- (void)_openGalleryWithAsset:(TGMediaPickerAsset *)asset
{
    ...
    if (self.textType == RecentItemTextTypeAttach) {
        [model.interfaceView setDoneButtonTextType:RecentItemTextTypeAttach];
    }
...
- (NSString *)stringForSendPhotos:(NSUInteger)count
{
    NSString *format = @"";
    if (self.textType == RecentItemTextTypeAttach) {
        format = TGLocalized(@"QuickAttach.Photos_any");
        if (count == 1)
            format =  TGLocalized(@"QuickAttach.Photos_1");
        else if (count == 2)
            format =  TGLocalized(@"QuickAttach.Photos_2");
        else if (count >= 3 && count <= 10)
            format =  TGLocalized(@"QuickAttach.Photos_3_10");
    }else{
        format = TGLocalized(@"QuickSend.Photos_any");
        if (count == 1)
