...
- (void)controllerWantsToPostMessages:(NSArray *)messageIds groupAvatar:(UIImage *)groupAvatar
{
    [TGModernConversationCompanion dispatchOnMessageQueue:^
     {
         NSMutableArray *messages = [[NSMutableArray alloc] init];
         ...
         TGDispatchOnMainThread(^
                                {
                                    TGForwardStreamController *forwardController = [[TGForwardStreamController alloc] initWithPostMessages:messages conversationId:_conversationId groupAvatar:groupAvatar];
                                    TGNavigationController *navigationController = [TGNavigationController navigationControllerWithRootController:forwardController];
...
- (BOOL)checkedMessageType:(NSArray *)msgIds
{
    NSMutableArray *messageIds = [NSMutableArray arrayWithArray:msgIds];
    ...
    for (TGMessage *message in messages)
    {
        for (TGMediaAttachment *attachment in message.mediaAttachments)
        {
            if (attachment.type == TGImageMediaAttachmentType)
            {
                ...
                TGDocumentMediaAttachment *documentAttachment = (TGDocumentMediaAttachment *)attachment;
                if ([documentAttachment.mimeType isEqualToString:@"image/gif"]) {
                    imageCount ++;
                }
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    ...
    else if ([action isEqualToString:@"userAvatarLongPress"])
    {
        TGModernConversationController *controller = self.controller;
        
        [controller setInputTextPanelAtUser:[options[@"uid"] intValue]];
    }
...
- (void)_replaceMessages:(NSArray *)newMessages
{
    if (_conversationId < 0) {
        ...
        [newMessages enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL __unused *stop) {
            TGMessage *message = (TGMessage *)obj;
            NSString *key = [T8Common createMessageKeyWithMid:message.mid fromUid:message.fromUid toUid:message.toUid date:message.date];
            if (idx == 0) {
                ids = [ids stringByAppendingString:key];
            }else{
                ids = [ids stringByAppendingFormat:@",%@",key];
            }
            if ((message.actionInfo.actionType == TGMessageActionChatEditPhoto || message.actionInfo.actionType == TGMessageActionChatEditTitle) && message.unread) {
                needUpdate = YES;
            }
        }];
        //如果发现有修改了群组名称和头像的消息，则向服务器更新该群组的信息
        if (needUpdate) {
            TGConversation *conversation = [TGDatabaseInstance() loadConversationWithIdCached:_conversationId];
            [T8GroupAndCommunityService createCommunityWithThirdGroupID:conversation.conversationId createrID:T8CONTEXT.t8UserId image:nil memberCount:conversation.chatParticipantCount privilege:GroupDiscoverPrivilegeUnknow name:conversation.chatTitle description:nil language:nil imageKey:conversation.chatPhotoSmall success:^(NSDictionary __unused *dictRet) {
                
            } failure:^(NSDictionary __unused *dictRet, NSError __unused *error) {
                
            }];
        }
        [T8VoteService queryVoteInfo:newMessages ids:ids conversationID:_conversationId success:^(NSDictionary __unused *dictRet) {
            BOOL result = ((NSNumber *)[dictRet objectForKey:@"result"]).boolValue;
            if (result) {
                NSArray *messages = [dictRet objectForKey:@"data"];
                __block NSMutableArray *needToUpdate = [NSMutableArray array];
                [messages enumerateObjectsUsingBlock:^(id obj, NSUInteger __unused idx, BOOL __unused *stop) {
                    NSDictionary *messageDict = (NSDictionary *)obj;
                    int count = [messageDict intValueForKey:@"upvotes" withDefault:0] - [messageDict intValueForKey:@"downvotes" withDefault:0];
                    BOOL upvoted = [[messageDict objectForKey:@"upvoted"] boolValue];
                    BOOL downvoted = [[messageDict objectForKey:@"downvoted"] boolValue];
                    NSString *messageKey = [messageDict objectForKey:@"tg_message_key"];
                    BOOL changed = [TGDatabaseInstance() storeVoteInfoForMessageWithKey:messageKey count:count upVote:upvoted downVote:downvoted];
                    if (changed) {
                        [needToUpdate addObject:[messageDict stringForKey:@"tg_message_key" withDefault:@"unknown"]];
                    }
                }];
                [[NSNotificationCenter defaultCenter] postNotificationName:Notification_Key_Vote_Changed object:needToUpdate];
            }
        } failure:^(NSDictionary __unused *dictRet, NSError __unused *error) {
    ...
    [super _replaceMessages:newMessages];
}
...
- (void)_addMessages:(NSArray *)addedMessages animated:(bool)animated intent:(TGModernConversationAddMessageIntent)intent
{
    if (_conversationId < 0) {
        __block NSString *ids = @"";
        __block BOOL needUpdate = NO;
        [addedMessages enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL __unused *stop) {
            TGMessage *message = (TGMessage *)obj;
            NSString *key = [T8Common createMessageKeyWithMid:message.mid fromUid:message.fromUid toUid:message.toUid date:message.date];
            if (idx == 0) {
                ids = [ids stringByAppendingString:key];
        ...
        [T8VoteService queryVoteInfo:addedMessages ids:ids conversationID:_conversationId success:^(NSDictionary __unused *dictRet) {
            BOOL result = ((NSNumber *)[dictRet objectForKey:@"result"]).boolValue;
            if (result) {
                NSArray *messages = [dictRet objectForKey:@"data"];
                __block NSMutableArray *needToUpdate = [NSMutableArray array];
    ...
    [super _addMessages:addedMessages animated:animated intent:intent];
}
