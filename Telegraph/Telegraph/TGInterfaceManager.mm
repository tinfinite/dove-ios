...
- (void)navigateToConversationWithId:(int64_t)conversationId conversation:(TGConversation *)__unused conversation performActions:(NSDictionary *)performActions atMessage:(NSDictionary *)atMessage clearStack:(bool)clearStack openKeyboard:(bool)__unused openKeyboard animated:(bool)animated
{
                ...
                if (viewControllers.count > 1){
                    ...
                    [UIView animateWithDuration:0.5
                                     animations:^{
                                         ...
                                         [containingNavigationController setViewControllers:@[viewControllers[0], conversationController] animated:NO];
                                         [UIView setAnimationTransition:UIViewAnimationTransitionFlipFromRight forView:containingNavigationController.view cache:NO];
                                     }];
                    
                    if (performActions[@"forwardQRCode"] != 0)
                    {
                        NSDictionary *imageDescription = [(TGGenericModernConversationCompanion *)conversationController.companion imageDescriptionFromImage:performActions[@"forwardQRCode"] optionalAssetUrl:nil];
                        NSArray *imageDescriptions = [NSArray arrayWithObject:imageDescription];
                        [(TGGenericModernConversationCompanion *)conversationController.companion controllerWantsToSendImagesWithDescriptions:imageDescriptions];
                    }
                }
                else{
    ...
    else
    {
        ...
        dict = performActions[@"forwardMessages"];
        if ([dict count] != 0)
...
- (void)navigateToShareQRCodeOfConversation:(TGConversation *)conversation navigationController:(UINavigationController *)navigationController
{
    TGGroupInfoQRCodeController *controller = [[TGGroupInfoQRCodeController alloc] initConversation:conversation];

    [navigationController pushViewController:controller animated:true];
}

- (void)navigateToJoinRequestOfConversation:(TGConversation *)conversation navigationController:(UINavigationController *)navigationController
{
    TGGroupInfoJoinRequestController *controller = [[TGGroupInfoJoinRequestController alloc] initConversation:conversation];
    
    [navigationController pushViewController:controller animated:true];
}

- (void)navigateToUpvoteMessageOfConversation:(TGConversation *)conversation navigationController:(UINavigationController *)navigationController
{
    TGGroupInfoUpvoteMessageController *controller = [[TGGroupInfoUpvoteMessageController alloc] initConversation:conversation];
