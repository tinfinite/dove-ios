...
- (void)setMode:(TGFlatActionCellMode)mode
{
    ...
    else if (mode == TGFlatActionCellModeJoinGroup)
        _titleLabel.text = TGLocalized(@"GroupInfo.JoinGroup");
    ...
    static UIImage *joinGroupIcon = nil;
    ...
    if (inviteIcon == nil)
    {
        ...
        joinGroupIcon = [UIImage imageNamed:@"group_join_scanQR"];
    }
    ...
    {
        _iconView.image = [joinGroupIcon imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
        [_iconView sizeToFit];
        
        CGRect iconFrame = _iconView.frame;
        iconFrame.origin = CGPointMake(20 - TGRetinaPixel + horizontalOffset - 1 + 4, 8 + verticalOffset + 3);
        _iconView.frame = iconFrame;
    }
}
