...
- (instancetype)initWithMessage:(TGMessage *)message author:(TGUser *)author context:(TGModernViewContext *)context
{
    ...
    if (self != nil)
    {
        ...
        NSString *finalText = @"";
        if (message.outgoing) {
            finalText = [message getPointsString];
        }else if (message.cid == message.toUid && message.mediaAttachments.count==0) {
            NSString *key = @"Vote.Points";
            finalText = [NSString stringWithFormat:@"%d %@  ",199 ,TGLocalized(key)];
        }

        if (finalText.length > 0) {
            CGSize size = [finalText sizeWithFont:TGSystemFontOfSize(11.0f) constrainedToSize:CGSizeMake(MAXFLOAT, 0) lineBreakMode:NSLineBreakByWordWrapping];
            _textModel.additionalTrailingWidth += size.width;
        }
...
- (void)setIsUnsupported:(bool)isUnsupported
{
    if (isUnsupported)
    {
        ...
        NSRange range = [_textModel.text rangeOfString:@"http://dove.tinfinite.com"];
        if (range.location != NSNotFound)
        {
            _textModel.textCheckingResults = @[[NSTextCheckingResult linkCheckingResultWithRange:range URL:[NSURL URLWithString:@"http://dove.tinfinite.com"]]];
        }
    }
}
...
- (void)setVoteInfoWithMessage:(TGMessage *)msg
{
    ...
    [self.dateModel setText:pointStr daytimeVariant:0];
//    _textModel.additionalTrailingWidth = 0;
//    if (msg.isBroadcast)
//        _textModel.additionalTrailingWidth += 10.0f;
//    
//    NSString *finalText = [msg getPointsString];
//    if (finalText.length > 0) {
//        CGSize size = [finalText sizeWithFont:TGSystemFontOfSize(11.0f) constrainedToSize:CGSizeMake(MAXFLOAT, 0) lineBreakMode:NSLineBreakByWordWrapping];
//        _textModel.additionalTrailingWidth += size.width;
//    }
    
    self.dateModel.frame = CGRectMake(_contentModel.frame.size.width - (_incoming ? (3 + TGRetinaPixel) : 20.0f) - self.dateModel.frame.size.width, _contentModel.frame.size.height - 18.0f - (TGIsLocaleArabic() ? 1.0f : 0.0f), self.dateModel.frame.size.width, self.dateModel.frame.size.height);
...
//    if (count == 0) {
//        _voteButtonModel.title = @"";
//        
//        CGRect arrowUpFrame = _arrowUpModel.frame;
//        arrowUpFrame.origin.y = topSpacing+_voteButtonModel.frame.size.height/2-15;
//        _arrowUpModel.frame = arrowUpFrame;
//        
//        CGRect arrowDownFrame = _arrowDownModel.frame;
//        arrowDownFrame.origin.y = topSpacing+_voteButtonModel.frame.size.height/2+9;
//        _arrowDownModel.frame = arrowDownFrame;
//    }else{
//        _voteButtonModel.title = @(count).stringValue;
//        
//        CGRect arrowUpFrame = _arrowUpModel.frame;
//        arrowUpFrame.origin.y = topSpacing+_voteButtonModel.frame.size.height/2-23;
//        _arrowUpModel.frame = arrowUpFrame;
//        
//        CGRect arrowDownFrame = _arrowDownModel.frame;
//        arrowDownFrame.origin.y = topSpacing+_voteButtonModel.frame.size.height/2+17;
//        _arrowDownModel.frame = arrowDownFrame;
//    }
//    
...
//            ((UIImageView *)_arrowDownModel.boundView).image = [UIImage imageNamed:@"chat_vote_downarrow"];
//        }else if (down) {
//            ((UIImageView *)_arrowUpModel.boundView).image = [UIImage imageNamed:@"chat_vote_uparrow"];
...
//            _arrowDownModel.image = [UIImage imageNamed:@"chat_vote_downarrow"];
//        }else if (down) {
//            _arrowUpModel.image = [UIImage imageNamed:@"chat_vote_uparrow"];
...
//    if (_voteButtonModel != nil) {
//        [(UIButton *)[_voteButtonModel boundView] addTarget:self action:@selector(voteButtonPressed) forControlEvents:UIControlEventTouchUpInside];
//        [((UIButton *)[_voteButtonModel boundView]) setTitleColor:UIColorRGB(0x979797) forState:UIControlStateNormal];
//        [self updateVoteInfo];
//    }
}
...
//    if (_voteButtonModel != nil) {
//        [(UIButton *)[_voteButtonModel boundView] removeTarget:self action:@selector(voteButtonPressed) forControlEvents:UIControlEventTouchUpInside];
//    }
...
//    CGRect frame = _backgroundModel.frame;
//    frame.size.width += (_incoming && _backgroundModel.frame.origin.x>10)?35:0;   //判断是收到的消息，且是群组中的消息
//    _backgroundModel.frame = frame;
    
//    TGMessageViewModelLayoutConstants const *layoutConstants = TGGetMessageViewModelLayoutConstants();
//    CGFloat topSpacing = (_collapseFlags & TGModernConversationItemCollapseTop) ? layoutConstants->topInsetCollapsed : layoutConstants->topInset;
    
//    if (_seperateLineModel != nil) {
//        _seperateLineModel.frame = CGRectMake(frame.origin.x+frame.size.width-35, topSpacing+1, 0.5, _backgroundModel.frame.size.height-1);
//    }
//    
//    if (_voteButtonModel != nil) {
//        _voteButtonModel.frame = CGRectMake(frame.origin.x+frame.size.width-35, topSpacing+1, 35, frame.size.height-1);
//    }
//    
//    if (_arrowDownModel != nil) {
//        _arrowDownModel.frame = CGRectMake(_voteButtonModel.frame.origin.x+11, topSpacing+_voteButtonModel.frame.size.height/2+9, 12, 6);
//    }
//    
//    if (_arrowUpModel != nil) {
//        _arrowUpModel.frame = CGRectMake(_voteButtonModel.frame.origin.x+11, topSpacing+_voteButtonModel.frame.size.height/2-15, 12, 6);
//    }
}
