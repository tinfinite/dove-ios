...
@interface TGModernConversationController () <UICollectionViewDataSource, TGModernConversationViewLayoutDelegate, UIViewControllerTransitioningDelegate, HPGrowingTextViewDelegate, UIGestureRecognizerDelegate, TGImagePickerControllerDelegate, TGLegacyCameraControllerDelegate, TGModernConversationInputTextPanelDelegate, TGModernConversationEditingPanelDelegate, TGModernConversationPostPanelDelegate, TGModernConversationTitleViewDelegate, TGForwardContactPickerControllerDelegate, TGModernConversationAudioPlayerDelegate, TGAudioRecorderDelegate,TGSelectAtUserControllerDelegate, NSUserActivityDelegate,AGEmojiKeyboardViewDelegate>
{
...
@property (nonatomic,strong) AGEmojiKeyboardView *emojiView;
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        _avatarButton = [[TGModernConversationAvatarButton alloc] initWithFrame:CGRectMake(0, 0, 25, 25)];
        ...
        [self setRightBarButtonItems:[self defaultRightBarButtonItem]];
...
- (NSArray *)defaultRightBarButtonItem
{
//    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone)
    if ([_companion isKindOfClass:[TGGroupModernConversationCompanion class]]) {
        TGGroupModernConversationCompanion *companion = (TGGroupModernConversationCompanion *)_companion;
        UIButton *boardButton = [UIButton buttonWithType:UIButtonTypeCustom];
        boardButton.frame = CGRectMake(0, 0, 25, 25);
        [boardButton setBackgroundImage:[UIImage imageNamed:@"group_board_white"] forState:UIControlStateNormal];
        [boardButton addTarget:self action:@selector(groupBoardPressed) forControlEvents:UIControlEventTouchUpInside];
        
        UIView *unreadView = [[UIView alloc] init];
        unreadView.frame = CGRectMake(21, -2, 8, 8);
        ...
        [boardButton addSubview:unreadView];
        
        UIBarButtonItem *boardButtonItem = [[UIBarButtonItem alloc] initWithCustomView:boardButton];
        
        [T8GroupHttpRequestService getGroupBoardUnreadCountWithGroupId:@(ABS(companion.conversationId)).stringValue success:^(NSDictionary *dictRet) {
            dispatch_async(dispatch_get_main_queue(), ^{
                if ([dictRet[@"count"] integerValue] > 0) {
        ...
        return @[_avatarButtonItem , boardButtonItem];
    }
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    if (false && iosMajorVersion() >= 8 && _currentActivity == nil && userActivityData != nil)
    {
        ...
        _currentActivity.webpageURL = [NSURL URLWithString:@"http://dove.tinfinite.com"];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [tracker set:kGAIScreenName value:@"聊天页面"];
...
//    if ([_inputTextPanel.maybeInputField.text isEqualToString:@"@"]) {
//        [_inputTextPanel.maybeInputField.internalTextView deleteBackward];
//    }
...
- (void)groupBoardPressed
{
    ...
    [_companion _controllerGroupBoardPressed];
}
...
- (void)_updateEditingPanel
{
    if ([_currentInputPanel isKindOfClass:[TGModernConversationEditingPanel class]])
    {
        ...
        [editingPanel setPostActionEnabled:[_companion checkedMessageType:[_companion checkedMessageIds]]];
    }
}
...
- (void)highlightAndShowActionsMenuForMessage:(int32_t)messageId
{
    ...
    for (TGModernCollectionCell *cell in _collectionView.visibleCells)
    {
        ...
        if (messageItem != nil && messageItem->_message.mid == messageId)
        {
            ...
            NSMutableArray *actions = [[NSMutableArray alloc] init];
            [actions addObject:[[NSDictionary alloc] initWithObjectsAndKeys:TGLocalized(@"Conversation.ContextMenuPost"), @"title", @"post", @"action", nil]];
            ...
            if (messageItem->_message.outgoing == false && messageItem->_message.cid == messageItem->_message.toUid) {
                BOOL textCanVote = messageItem->_message.mediaAttachments.count==0;
                BOOL imageCanVote = NO;
                ...
                BOOL canVote = textCanVote || imageCanVote;
                if (canVote) {
                    [actions addObject:[[NSDictionary alloc] initWithObjectsAndKeys:TGLocalized(@"Conversation.ContextMenuVote"), @"title", @"vote", @"action", nil]];
                }
            }
...
- (void)setInputTextPanelAtUser:(int)uid
{
    _inputTextPanel.inputField.internalTextView.enableFirstResponder = true;
    
    TGUser *user = [TGDatabaseInstance() loadUser:uid];
    
    NSString *inputString = [_inputTextPanel.inputField.text stringByAppendingString:[NSString stringWithFormat:@"@%@ %@[%@] ",user.firstName,user.lastName,@(uid)]];
    
    [[_inputTextPanel inputField] setText:inputString animated:NO];
}
...
- (void)editingPanelRequestedPostMessages:(TGModernConversationEditingPanel *)__unused editingPanel
{
    [_companion controllerWantsToPostMessages:[_companion checkedMessageIds] groupAvatar:[_avatarButton avatarImage]];
}

- (void)postPanelRequestedPostMessages:(TGModernConversationPostPanel *)__unused postPanel
{
    if ([_companion checkedMessageType:[_companion checkedMessageIds]]) {
        [_companion controllerWantsToPostMessages:[_companion checkedMessageIds] groupAvatar:[_avatarButton avatarImage]];
    }else{
...
- (void)inputPanelTextChanged:(TGModernConversationInputTextPanel *)__unused inputTextPanel text:(NSString *)text
{
    ...
    if (_alreadyHadWillAppear) {
        [_companion _controllerSelectAtController:text];
    }
...
- (void)inputPanelRequestedKeyboardSwitch:(TGModernConversationInputTextPanel *)__unused inputTextPanel emojiBoard:(BOOL)emoji
{
    if (emoji) {
        _inputTextPanel.inputField.internalTextView.inputView = self.emojiView;
    }else{
        _inputTextPanel.inputField.internalTextView.inputView = nil;
    }
    [_inputTextPanel.inputField.internalTextView reloadInputViews];
    _inputTextPanel.inputField.internalTextView.enableFirstResponder = true;
    [_inputTextPanel.inputField.internalTextView becomeFirstResponder];
}
...
- (void)_enterEditingMode:(int32_t)animateFromMessageId
{
    if (!_editingMode)
    {
        ...
        [self setRightBarButtonItems:@[[[UIBarButtonItem alloc] initWithTitle:TGLocalized(@"Common.Cancel") style:UIBarButtonItemStylePlain target:self action:@selector(doneButtonPressed)]] animated:true];
...
- (void)postEditingMode:(int32_t)animateFromMessageId
{
    if (!_editingMode)
    {
        [self setCurrentTitlePanel:nil animation:TGModernConversationPanelAnimationSlide];
        ...
        TGModernConversationPostPanel *editPanel = [[TGModernConversationPostPanel alloc] initWithFrame:CGRectMake(0.0f, 0.0f, self.view.frame.size.width, 0.0f)];
        editPanel.delegate = self;
        [self setInputPanel:editPanel animated:true];
        [self _updateEditingPanel];
...
- (void)_leaveEditingModeAnimated:(bool)animated
{
    if (_editingMode)
    {
        ...
        [self setRightBarButtonItems:[self defaultRightBarButtonItem] animated:animated];
...
//                [_collectionView setBounds:CGRectMake(0, contentOffset.y, _collectionView.frame.size.width, _collectionView.frame.size.height)];
                //qi.zhang modify, fix chat bubble slide up problem
                _collectionView.contentOffset = CGPointMake(0, contentOffset.y);
            }
    ...
    else
    {
        if (!CGPointEqualToPoint(contentOffset, originalContentOffset)){
//            [_collectionView setBounds:CGRectMake(0, contentOffset.y, _collectionView.frame.size.width, _collectionView.frame.size.height)];
            //qi.zhang modify, modify it when fix chat bubble slide up, no sure why modify this
            _collectionView.contentOffset = CGPointMake(0, contentOffset.y);
        }
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    ...
    else if ([action isEqualToString:@"menuAction"])
    {
        ...
        if (mid != 0)
        {
            ...
            if ([menuAction isEqualToString:@"post"]) {
                [self postEditingMode:mid];
            ...
            else if ([menuAction isEqualToString:@"vote"]){
                [self votePressedWithItem:menuMessageItem];
            }
        }
    }
...
- (void)votePressedWithItem:(TGMessageModernConversationItem *)item
{
    TGMessage *messageWithLatestVoteInfo = [TGDatabaseInstance() loadMessageWithMid:item->_message.mid];
    TGVoteInfoObject *obj = [messageWithLatestVoteInfo.contentProperties objectForKey:StoreKey_MessageVoteInfo];
    TGVoteView *voteView = [[TGVoteView alloc] initWithUpvoteStatus:obj.upvote downvoteStatus:obj.downvote upvoteBlock:^{
        NSString *msgKey = [T8Common createMessageKeyWithMid:messageWithLatestVoteInfo.mid fromUid:messageWithLatestVoteInfo.fromUid toUid:messageWithLatestVoteInfo.toUid date:messageWithLatestVoteInfo.date];
        [T8VoteService upvoteMessage:messageWithLatestVoteInfo.mid messageKey:msgKey cid:messageWithLatestVoteInfo.cid text:messageWithLatestVoteInfo.text success:^(NSDictionary *dictRet) {
            [T8Common storeVoteInfoForMessage:messageWithLatestVoteInfo.mid successData:dictRet];
            TGMessage *latest = [TGDatabaseInstance() loadMessageWithMid:item->_message.mid];
            [((TGMessageViewModel *)item.viewModel) setVoteInfoWithMessage:latest];
        } failure:^(NSDictionary __unused *dictRet, NSError __unused *error) {
            
        }];
    } downvoteBlock:^{
        NSString *msgKey = [T8Common createMessageKeyWithMid:messageWithLatestVoteInfo.mid fromUid:messageWithLatestVoteInfo.fromUid toUid:messageWithLatestVoteInfo.toUid date:messageWithLatestVoteInfo.date];
        [T8VoteService downvoteMessage:messageWithLatestVoteInfo.mid messageKey:msgKey cid:messageWithLatestVoteInfo.cid text:messageWithLatestVoteInfo.text success:^(NSDictionary __unused *dictRet) {
            [T8Common storeVoteInfoForMessage:messageWithLatestVoteInfo.mid successData:dictRet];
            TGMessage *latest = [TGDatabaseInstance() loadMessageWithMid:item->_message.mid];
            [((TGMessageViewModel *)item.viewModel) setVoteInfoWithMessage:latest];
        } failure:^(NSDictionary __unused *dictRet, NSError __unused *error) {
...
{
    if (!_emojiView) {
        _emojiView = [[AGEmojiKeyboardView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, 216)];
        _emojiView.delegate = self;
        _emojiView.autoresizingMask = UIViewAutoresizingFlexibleHeight;
    }
    return _emojiView;
}
...
{
    NSString *text = _inputTextPanel.inputField.internalTextView.text;
    _inputTextPanel.inputField.internalTextView.text = [text stringByAppendingString:emoji];
}

- (void)emojiKeyBoardViewDidPressBackSpace:(AGEmojiKeyboardView *)__unused emojiKeyBoardView
{
    [_inputTextPanel.inputField.internalTextView deleteBackward];
}
