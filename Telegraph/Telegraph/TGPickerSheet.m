...
- (id)initWithPickerSheetType:(PickerSheetType)type;
...
- (id)init
{
    return [self initWithPickerSheetType:PickerSheetTypeTime];
}
...
{
    if (_type == PickerSheetTypeTime) {
        TGSecretTimerValueControllerItemView *oldView = (TGSecretTimerValueControllerItemView *)view;
        if (oldView != nil)
        {
            oldView.emptyValue = _emptyValue;
            oldView.seconds = [_timerValues[row] intValue];
            return oldView;
        }
        ...
        return newView;
    }else if (_type == PickerSheetTypeLanguage){
        TGPickerSheetItemView *oldView = (TGPickerSheetItemView *)view;
        if (oldView != nil) {
            oldView.title = _timerValues[row];
            return oldView;
        }
        
        TGPickerSheetItemView *newView = [[TGPickerSheetItemView alloc] init];
        newView.title = _timerValues[row];
        return newView;
    }
    return nil;
}
...
{
    return [self initWithItems:items selectedIndex:selectedIndex type:PickerSheetTypeTime action:action];
}

- (instancetype)initWithItems:(NSArray *)items selectedIndex:(NSUInteger)selectedIndex type:(PickerSheetType)type action:(void (^)(id))action
{
    self = [super init];
    if (self != nil)
    {
...
- (void)show
{
    if (_controllerWindow == nil)
    {
        ...
        _controllerWindow.rootViewController = [[TGPickerSheetOverlayController alloc] initWithPickerSheetType:_type];
