...
@property (nonatomic, strong) NSMutableDictionary *loadedConversationIDs;
@property (nonatomic, assign) int loadedConversationDate;
...
- (id)init
{
    ...
    if (self != nil)
    {
        ...
        _loadedConversationIDs = [NSMutableDictionary dictionary];
        _loadedConversationDate = INT_MAX;
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(handleStickChangedNotification:) name:Notification_Key_Stick_Changed object:nil];
    }
    return self;
}
...
- (void)dealloc
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:Notification_Key_Stick_Changed object:nil];
}
...
- (void)resetWatchedNodePaths
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        ...
        [ActionStageInstance() watchForPath:@"/tg/unreadCountForGroup" watcher:self];
        [ActionStageInstance() watchForPath:@"/tg/unreadCountForSingle" watcher:self];
        ...
        [self actionStageResourceDispatched:@"/tg/unreadCountForGroup" resource:[[SGraphObjectNode alloc] initWithObject:[NSNumber numberWithInt:unreadCountForGroup]] arguments:nil];
        int unreadCountForSingle = [TGDatabaseInstance() databaseState].unreadCountForSingle;
        [self actionStageResourceDispatched:@"/tg/unreadCountForSingle" resource:[[SGraphObjectNode alloc] initWithObject:[NSNumber numberWithInt:unreadCountForSingle]] arguments:nil];
    }];
}
...
- (void)loadMoreItems
{
    [ActionStageInstance() dispatchOnStageQueue:^
    {
        NSMutableArray *currentConversationIds = [[NSMutableArray alloc] initWithCapacity:_loadedConversationIDs.count];
        
        int minDate = INT_MAX;
        if (_loadedConversationDate < minDate) {
            minDate = _loadedConversationDate;
        }
        [currentConversationIds addObjectsFromArray:_loadedConversationIDs.allValues];
        for (NSNumber *conversationID in _loadedConversationIDs.allValues)
        {
            [currentConversationIds addObject:conversationID];
        }
...
//                [_conversationList addObjectsFromArray:loadedItems];
                [self addConversations:loadedItems];
...
//                [_conversationList addObjectsFromArray:loadedItems];
            ...
            //qi.zhang modify
            [self sortConversation:_conversationList];
...
//                [_conversationList addObject:newConversation];
                [self addConversations:@[newConversation]];
            }
        }
...
//         {
//             if ([((TGConversation *)obj1).chatTitle isEqualToString:@"t8"] && ![((TGConversation *)obj2).chatTitle isEqualToString:@"t8"]) {
//                 return NSOrderedAscending;
//             }else if (![((TGConversation *)obj1).chatTitle isEqualToString:@"t8"] && [((TGConversation *)obj2).chatTitle isEqualToString:@"t8"]){
//                 return NSOrderedDescending;
//             }
//             //qi.zhang to change
//             int date1 = (int)((TGConversation *)obj1).date;
...
//                    [TGAppDelegateInstance.mainTabsController setUnreadCountForGroup:unreadCount];
//                    [TGAppDelegateInstance.mainTabsController setUnreadCountForSingle:unreadCount];
    ...
    else if ([path isEqualToString:@"/tg/unreadCountForGroup"])
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [TGDatabaseInstance() dispatchOnDatabaseThread:^{
                int unreadCountForGroup = [TGDatabaseInstance() databaseState].unreadCountForGroup;
                TGDispatchOnMainThread(^{
                    self.unreadCountForGroup = unreadCountForGroup;
                    [TGAppDelegateInstance.mainTabsController setUnreadCountForGroup:unreadCountForGroup];
                });
            } synchronous:false];
        });
    }
    else if ([path isEqualToString:@"/tg/unreadCountForSingle"])
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            [TGDatabaseInstance() dispatchOnDatabaseThread:^{
                int unreadCountForSingle = [TGDatabaseInstance() databaseState].unreadCountForSingle;
                TGDispatchOnMainThread(^{
                    self.unreadCountForSingle = unreadCountForSingle;
                    [TGAppDelegateInstance.mainTabsController setUnreadCountForSingle:unreadCountForSingle];
                });
            } synchronous:false];
        });
    }
...
//qi.zhang add, sort conversation with stick info
- (void)sortConversation:(NSMutableArray *)conversations
{
    [conversations sortUsingComparator:^NSComparisonResult(id obj1, id obj2)
     {
//         BOOL obj1_stick = [[T8StickToTopManager sharedT8StickToTopManager] checkIsStickedWithConversationID:((TGConversation *)obj1).conversationId andUserID:((TGConversation *)obj1).conversationId];
         BOOL obj1_stick = [[T8StickToTopManager sharedInstance] checkConversationIsStickedWithID:[NSString stringWithFormat:@"%lld",((TGConversation *)obj1).conversationId]];
//         BOOL obj2_stick = [[T8StickToTopManager sharedT8StickToTopManager] checkIsStickedWithConversationID:((TGConversation *)obj2).conversationId andUserID:((TGConversation *)obj2).conversationId];
         BOOL obj2_stick = [[T8StickToTopManager sharedInstance] checkConversationIsStickedWithID:[NSString stringWithFormat:@"%lld",((TGConversation *)obj2).conversationId]];
         if (obj1_stick && !obj2_stick) {
             return NSOrderedAscending;
         }else if (!obj1_stick && obj2_stick){
             return NSOrderedDescending;
         }else if (obj1_stick && obj2_stick){
             return [[T8StickToTopManager sharedInstance] compareConversationID:[NSString stringWithFormat:@"%lld",((TGConversation *)obj1).conversationId] with:[NSString stringWithFormat:@"%lld",((TGConversation *)obj2).conversationId]];
         }else{
             int date1 = (int)((TGConversation *)obj1).date;
             int date2 = (int)((TGConversation *)obj2).date;
...
- (void)handleStickChangedNotification:(NSNotification *) __unused notification
{
    [self sortConversation:_conversationList];
    NSArray *items = [NSArray arrayWithArray:_conversationList];
    dispatch_async(dispatch_get_main_queue(), ^{
        TGDialogListController *dialogListController = self.dialogListController;
        [dialogListController dialogListFullyReloaded:items];
    });
}

- (void)addConversations:(NSArray *)conversationItems
{
    [conversationItems enumerateObjectsUsingBlock:^(id obj, NSUInteger __unused idx, BOOL __unused *stop) {
        TGConversation *conversation = (TGConversation *)obj;
        [_loadedConversationIDs setObject:[[NSNumber alloc] initWithLongLong:conversation.conversationId] forKey:@(conversation.conversationId).stringValue];
        if (conversation.date < _loadedConversationDate && !conversation.isBroadcast)
            _loadedConversationDate = conversation.date;
        if (self.companionType == TGTelegraphDialogListCompanionTypeGroup) {
            if (conversation.chatTitle.length>0) {
                [_conversationList addObject:conversation];    
            }
        }else if (self.companionType == TGTelegraphDialogListCompanionTypeSingle) {
            if (conversation.chatTitle.length==0) {
                [_conversationList addObject:conversation];
            }
        }else if (self.companionType == TGTelegraphDialogListCompanionTypeAll){
            [_conversationList addObject:conversation];
        }
    }];
}
