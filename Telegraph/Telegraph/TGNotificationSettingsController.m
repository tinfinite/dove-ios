...
@interface TGNotificationSettingsController () <TGAlertSoundControllerDelegate>
{
    ...
    TGSwitchCollectionItem *_commentAlert;
    TGSwitchCollectionItem *_upvoteAlert;
    TGSwitchCollectionItem *_addGroupAlert;
...
- (id)init
{
    ...
    if (self)
    {
        ...
        NSDictionary *syncDict = [[NSUserDefaults standardUserDefaults] objectForKey:UserDefaultKey_UserSyncInfo];
        _commentAlert = [[TGSwitchCollectionItem alloc] initWithTitle:TGLocalized(@"Notifications.CommentAlert") isOn:[syncDict[@"is_notify_comment"] integerValue]];
        _commentAlert.interfaceHandle = _actionHandle;
        _upvoteAlert = [[TGSwitchCollectionItem alloc] initWithTitle:TGLocalized(@"Notifications.UpvoteAlert") isOn:[syncDict[@"is_notify_upvote"] integerValue]];
        _upvoteAlert.interfaceHandle = _actionHandle;
        _addGroupAlert = [[TGSwitchCollectionItem alloc] initWithTitle:TGLocalized(@"Notifications.ApplyGroupAlert") isOn:[syncDict[@"is_notify_group_apply"] integerValue]];
        _addGroupAlert.interfaceHandle = _actionHandle;
        TGCollectionMenuSection *eventNotificationsSection = [[TGCollectionMenuSection alloc] initWithItems:@[
             [[TGHeaderCollectionItem alloc] initWithTitle:TGLocalized(@"Notifications.EventNotification")],
             _commentAlert,
             _upvoteAlert,
             _addGroupAlert
        ]];
        [self.menuSections addSection:eventNotificationsSection];
...
{
    [T8UserHttpRequestService updateUserNotificationSettingsWithCommentNotify:_commentAlert.isOn upvoteNotify:_upvoteAlert.isOn applyGroupNotify:_addGroupAlert.isOn successBlock:^(NSDictionary __unused*dictRet) {
        NSMutableDictionary *syncDict = [[[NSUserDefaults standardUserDefaults] objectForKey:UserDefaultKey_UserSyncInfo] mutableCopy];
        [syncDict setObject:@(_commentAlert.isOn).stringValue forKey:@"is_notify_comment"];
        [syncDict setObject:@(_upvoteAlert.isOn).stringValue forKey:@"is_notify_upvote"];
        [syncDict setObject:@(_addGroupAlert.isOn).stringValue forKey:@"is_notify_group_apply"];
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    if ([action isEqualToString:@"switchItemChanged"])
    {
        ...
        else if (switchItem == _inAppVibrate)
        {
            ...
            [TGAppDelegateInstance saveSettings];
        }else if (switchItem == _inAppPreview)
        {
            TGAppDelegateInstance.bannerEnabled = switchItem.isOn;
            [TGAppDelegateInstance saveSettings];
        }else if (switchItem == _commentAlert){
            [self updateNotificationSettings];
        }else if (switchItem == _upvoteAlert){
            [self updateNotificationSettings];
        }else if (switchItem == _addGroupAlert){
            [self updateNotificationSettings];
        }
    }
}
