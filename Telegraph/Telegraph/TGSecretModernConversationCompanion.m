...
{
    return @{@"title": TGLocalized(@"Conversation.UnsecretChat"), @"action": @"unsecretChat"};
}
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    if ([action isEqualToString:@"actionPanelAction"])
    {
        ...
        if ([panelAction isEqualToString:@"unsecretChat"]) {
            [self startUnsecretChat];
        }
    }
