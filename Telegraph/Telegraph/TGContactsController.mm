...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    [tracker set:kGAIScreenName value:@"联系人Tab"];
...
static void adjustCellForUser(TGContactCell *contactCell, TGUser *user, int currentSortOrder, bool animated, std::map<int, TGUser *> const &selectedUsers, __unused bool showMessageBadge, bool isDisabled, bool isSearch, bool isGlobalSearch, NSString *searchString)
{
    ...
    if (currentSortOrder & TGContactListSortOrderDisplayFirstFirst)
    {
        ...
        //qi.zhang modify
        if ([user.firstName hasChinese] || [user.lastName hasChinese]) {
            [contactCell setBoldMode:1];
        }
        //
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (user != nil && (user.uid == INT_MAX || user.uid == INT_MAX - 1 || user.uid == INT_MAX - 2 || user.uid == INT_MAX - 3))
    {
        ...
        {
            mode = TGFlatActionCellModeJoinGroup;
        }
        else
            mode = TGFlatActionCellModeCreateGroup;
...
- (void)actorCompleted:(int)resultCode path:(NSString *)path result:(id)result
{
    ...
    else if ([path hasPrefix:@"/tg/conversation/"] && [path rangeOfString:@"/addMember/"].length > 0) {
        TGDispatchOnMainThread(^{
            NSRegularExpression *regular = [NSRegularExpression regularExpressionWithPattern:@"^/tg/conversation/\\((.*?)\\)/addMember/(.*?)$" options:NSRegularExpressionCaseInsensitive error:nil];
            NSUInteger matches = [regular numberOfMatchesInString:path options:0 range:NSMakeRange(0, path.length)];
            if (matches == 0) {
                return;
            }
            NSString *conversationIDStr = [regular stringByReplacingMatchesInString:path options:NSMatchingReportCompletion range:NSMakeRange(0, path.length) withTemplate:@"$1"];
            int64_t conversationID = (int64_t)conversationIDStr.intValue;
...
- (void)updateContactList
{
    ...
    if (_contactsMode & TGContactsModePhonebook)
    {
        ...
        else
        {
            for (TGPhonebookContact *phonebookContact in addressBook)
            {
                ...
                {
                    if (phonebookContact.firstName.length != 0)
                        sectionLetter = [phonetic(phonebookContact.firstName) characterAtIndex:0];
                    else if (phonebookContact.lastName.length != 0)
                        sectionLetter = [phonetic(phonebookContact.lastName) characterAtIndex:0];
                    else
                        sectionLetter = '#';
                }
                else
    ...
    else if ((_contactsMode & TGContactsModeMainContacts) == TGContactsModeMainContacts || ((_contactsMode & TGContactsModeCreateGroupOption) == TGContactsModeCreateGroupOption))
    {
        ...
        //if ((_contactsMode & TGContactsModeMainContacts) == TGContactsModeMainContacts)
        {
            TGUser *serviceUserJoinGroup = [[TGUser alloc] init];
            serviceUserJoinGroup.uid = INT_MAX - 3;
            serviceSection->addItem(serviceUserJoinGroup);
